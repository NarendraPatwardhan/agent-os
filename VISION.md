...
2. **Guest userland — coreutils flag fidelity.** The *language* is settled (§16.3): coreutils are **mc-native Rust**, one crate compiled per-tier — **not** uutils-via-WASI, **not** Toybox/BSD-via-`zig cc` (either would drag a WASI/libc lane into `/bin`; C/C++/Zig stays for the heavy *domain* tools alone — sqlite/luau/typst/duckdb). What remains genuinely per-tool is **GNU-flag fidelity**: agent-written scripts assume GNU semantics, so each applet decides how much of the GNU flag surface to implement — lifted from uutils' logic where it exists, hand-written otherwise — and the rarely-scripted long tail may lag.
...
### 16.3 `/bin` is a per-tier multicall — one mc-native Rust crate, compiled four times

memcontainers ships its Rust coreutils as **55 separate `.wasm`** (a ~19 MB layer) while *already* collapsing its WASI tools into per-tier multicalls (`mcbox-ro`/`mcbox-rw`: one binary, `argv[0]` dispatch, an `mc_applets` custom-section roster). agent-os keeps the multicall and **removes the WASI lane from `/bin` entirely**: every coreutil — whether its logic is lifted from uutils or hand-written — is **mc-native Rust**, compiled against `//sysroot` (the generated `mc` imports + the ported safe wrappers). No WASI, no relink, no libc-over-mc shim appears anywhere in `/bin`; that machinery (§13.4, the wasi-adapter) exists *only* for the C/C++ **domain** tools (§16.5), which are individually large one-binary services, never multicalled.

So `/bin` is **one `coreutils` crate, compiled four times** — which is uutils' own architecture (one crate, feature-gated applet modules, an `argv[0]` multicall `main`), retargeted from `std` to `//sysroot`. The four boxes — `mcbox-{isolated,readonly,readwrite,full}` — are four `rust_binary`s over one `srcs` glob, differing only by **crate features** (`cfg`): each applet module is gated to the lowest tier whose capabilities cover it, and a registration macro builds *both* the dispatch table and the `mc_applets` roster from whatever is compiled in — so a box's code, its roster, and its generated `/bin` symlinks cannot disagree (the symlink set is generated from the roster, never hand-listed). `/bin/cat`, `/bin/echo`, … are tar symlinks into their tier's box.

Two properties fall out by construction, not by cleverness:

- **One crate is one runtime.** A single mc-native crate has one allocator, one `#[panic_handler]`, one `_start`. The "two runtimes collide, so a box can't mix sources" problem a WASI lane would create *does not exist* — there is one lane.
- **Tier membership is the capability contract, not a hand-list.** An applet's tier *is* the lowest tier whose `tier_caps` (§15.4) ⊇ the capabilities its `mc` imports require (the `SYSCALL_CAPS` matrix, §15.4, walked by the import oracle of §9.3/§16.4). The `cfg` gate is the authoring knob; the §16.4 attestation is the *same computation run as a check* — a `readonly` box that compiles in a write applet fails the build, because its imports exceed `tier_caps(readonly)`.

The wins are threefold: the coreutils layer drops from ~19 MB to a few; 55 guest compiles become 4; and — the lever proven on the host's module cache — a session cranelift-compiles **four distinct guest modules**, not dozens (a *boot-time* size/speed lever, B5-adjacent). Some applets pull a third-party Rust crate (a regex engine for `sed`/`grep`, say); those enter by `http_archive`+patches (B3) and link into the same mc-native crate — still no WASI, just a patched dependency made freestanding-clean.
