...
2. **Guest userland language.** Phase A ports memcontainers' Rust `/bin`. Post-stabilization: keep uutils-via-WASI (permissive, GNU-compatible, but Rust), move hot tools to Zig, or convert **Toybox (0BSD)** / a **BSD userland (NetBSD, BSD-licensed, portable)** through `zig cc` for the long tail. The GNU-flag-compat axis matters for agent-written scripts; decide per-tool.
