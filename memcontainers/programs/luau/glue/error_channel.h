// error_channel.h — a kernel-trap-backed replacement for the handful of C++ try/catch/throw sites
// in Luau's Parser and Compiler (was loom/src/mc_eh.h). Stays C++: it is a C++ template included
// into the patched Luau source, so it cannot be Zig — but the unwind it rides on lives in trap.zig.
//
// WHY THIS EXISTS
// --------------
// The Luau guest is built `-fno-exceptions -fno-rtti` (zig's wasm32-wasi libc++ ships no C++
// exception runtime; the kernel's wasmi implements neither the EH proposal nor the setjmp/longjmp
// it needs). So the kernel provides the unwind, via trap.h's mc_protected_call / mc_raise. The
...
// error), reclaimed at process exit (script mode) and bounded by `mc_budget.mem` in the REPL. See
...
template <typename T> struct Channel {
    ...
    // Run `body()` under a kernel trap boundary. Returns true if `body` raised on THIS channel
    // (then call take()), false if it completed normally. The innermost mc_protected_call catches
