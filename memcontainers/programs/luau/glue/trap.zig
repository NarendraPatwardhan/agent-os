...
//! Luauc's patched VM and error channel reach these through its neutral runtime contract.
...
/// Run fn(ud) under a kernel trap boundary. Returns 0 if fn returned normally, else the raise
/// code. Re-entrant: a protected call nested inside fn saves/restores the prior thunk.
export fn luauc_runtime_v1_protected_call(func: *const fn (?*anyopaque) callconv(.c) void, ud: ?*anyopaque) c_int {
...
/// it as a fresh nested guest call. Runs the stashed thunk; a runtime raise traps back past here
...
/// Record `code`, then trap to the nearest protected-call boundary. Never returns (wasmi unwinds
/// the native stack to the nearest `mc_sys_pcall`).
export fn luauc_runtime_v1_raise(code: c_int) noreturn {
