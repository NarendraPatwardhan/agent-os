//! Production AgentOS entry for the first complete strict-AOT Proto graph.
//!
//! The root returns a compiled caller closure. That caller materializes its compiled child, invokes
//! it through the fixed CALL protocol, takes the generic arithmetic slow path for raw argv strings,
//! and rejoins generated code before returning one TValue.
...
pub export fn __main_argc_argv(argc: c_int, argv_pointer: [*][*:0]u8) c_int {
    if (argc != 3)
        return fail("usage: luau-aot-compiled-call LHS RHS\n", 2);

    const state = luaL_newstate() orelse return fail("luau-aot-compiled-call: state allocation failed\n", 1);
    ...
    if (abi.mc_luau_aot_v1_push_program(
        ...
        return fail("luau-aot-compiled-call: program publication failed\n", 1);

    // Keep both GC publication assertions in the production artifact: once while only the root
    // closure owns the Proto graph, and once while the returned caller closure owns its descendants.
    _ = lua_gc(state, lua_gc_collect, 0);
    if (lua_pcall(state, 0, 1, 0) != 0)
        return fail("luau-aot-compiled-call: root execution failed\n", 1);
    _ = lua_gc(state, lua_gc_collect, 0);
    ...
    if (lua_pcall(state, 2, 1, 0) != 0)
        return fail("luau-aot-compiled-call: nested execution failed\n", 1);
    ...
    if (is_number == 0 or !std.math.isFinite(result) or result != @trunc(result) or
        ...
        return fail("luau-aot-compiled-call: non-integer result\n", 1);

    var buffer: [32]u8 = undefined;
    const output = std.fmt.bufPrint(&buffer, "{d}\n", .{@as(i32, @intFromFloat(result))}) catch
        return fail("luau-aot-compiled-call: result formatting failed\n", 1);
