//! Production AgentOS entry for the first fixed multi-result strict-AOT Proto graph.
//!
//! The root returns a compiled caller closure. That caller invokes its compiled child for two
//! contiguous TValues, consumes both results, and returns their compiled arithmetic result.

const std = @import("std");
const abi = @import("luau_aot_runtime_abi");
const multi_result_call = @import("luau_aot_multi_result_call_program");
const mc = @import("mc");
...
pub export fn __main_argc_argv(argc: c_int, argv_pointer: [*][*:0]u8) c_int {
    if (argc != 3)
        return fail("usage: luau-aot-multi-result-call LHS RHS\n", 2);

    const state = luaL_newstate() orelse return fail("luau-aot-multi-result-call: state allocation failed\n", 1);
    defer lua_close(state);
    if (abi.mc_luau_aot_v1_push_program(
        state,
        &multi_result_call.program,
        multi_result_call.source_name.ptr,
        multi_result_call.source_name.len,
    ) != 0)
        return fail("luau-aot-multi-result-call: program publication failed\n", 1);

    _ = lua_gc(state, lua_gc_collect, 0);
    if (lua_pcall(state, 0, 1, 0) != 0)
        return fail("luau-aot-multi-result-call: root execution failed\n", 1);
    _ = lua_gc(state, lua_gc_collect, 0);
    ...
    if (lua_pcall(state, 2, 1, 0) != 0)
        return fail("luau-aot-multi-result-call: nested execution failed\n", 1);
    ...
    if (is_number == 0 or !std.math.isFinite(result) or result != @trunc(result) or
        ...
        return fail("luau-aot-multi-result-call: non-integer result\n", 1);

    var buffer: [32]u8 = undefined;
    const output = std.fmt.bufPrint(&buffer, "{d}\n", .{@as(i32, @intFromFloat(result))}) catch
        return fail("luau-aot-multi-result-call: result formatting failed\n", 1);
