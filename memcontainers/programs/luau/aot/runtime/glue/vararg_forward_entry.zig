//! Production AgentOS entry for the vararg-forwarding strict-AOT Proto graph.
//!
//! Four or five raw runtime strings enter an open vararg list. Its first value is consumed directly,
//! then the full list passes through a one-fixed-parameter variadic child and is adjusted into a
//! fixed four-parameter child before the saved first value is added to the result. The five-value
//! case proves that fixed-arity adjustment truncates the open tail.

const std = @import("std");
const abi = @import("luau_aot_runtime_abi");
const vararg_forward = @import("luau_aot_vararg_forward_program");
const mc = @import("mc");
...
pub export fn __main_argc_argv(argc: c_int, argv_pointer: [*][*:0]u8) c_int {
    if (argc != 5 and argc != 6)
        return fail("usage: luau-aot-vararg-forward A B C D [EXTRA]\n", 2);

    const state = luaL_newstate() orelse return fail("luau-aot-vararg-forward: state allocation failed\n", 1);
    defer lua_close(state);
    if (abi.mc_luau_aot_v1_push_program(
        state,
        &vararg_forward.program,
        vararg_forward.source_name.ptr,
        vararg_forward.source_name.len,
    ) != 0)
        return fail("luau-aot-vararg-forward: program publication failed\n", 1);

    _ = lua_gc(state, lua_gc_collect, 0);
    if (lua_pcall(state, 0, 1, 0) != 0)
        return fail("luau-aot-vararg-forward: root execution failed\n", 1);
    _ = lua_gc(state, lua_gc_collect, 0);

    for (1..@as(usize, @intCast(argc))) |index| {
        const argument = std.mem.span(argv_pointer[index]);
        lua_pushlstring(state, argument.ptr, argument.len);
    }
    if (lua_pcall(state, argc - 1, 1, 0) != 0)
        return fail("luau-aot-vararg-forward: nested execution failed\n", 1);
    ...
    if (is_number == 0 or !std.math.isFinite(result) or result != @trunc(result) or
        ...
        return fail("luau-aot-vararg-forward: non-integer result\n", 1);

    var buffer: [32]u8 = undefined;
    const output = std.fmt.bufPrint(&buffer, "{d}\n", .{@as(i32, @intFromFloat(result))}) catch
        return fail("luau-aot-vararg-forward: result formatting failed\n", 1);
