...
const aot = await instantiateOracle(
  runfile(process.env.LUAU_AOT_MULTI_RESULT_CALL_ORACLE_WASM, "LUAU_AOT_MULTI_RESULT_CALL_ORACLE_WASM"),
  "strict AOT multi-result-call oracle",
);
...
const source = readFileSync(
  runfile(process.env.LUAU_AOT_MULTI_RESULT_CALL_ORACLE_SOURCE, "LUAU_AOT_MULTI_RESULT_CALL_ORACLE_SOURCE"),
);

const aotInit = aot.exports.mc_luau_aot_v1_multi_result_call_oracle_init;
const aotRun = aot.exports.mc_luau_aot_v1_multi_result_call_oracle_run;
...
if ([aotInit, aotRun, interpreterInit, interpreterRun, interpreterSourceBuffer].some((item) => typeof item !== "function"))
  throw new Error("multi-result-call differential exports are incomplete");
...
for (const [lhs, rhs] of [[-50, 8], [0, 0], [20, 22], [7, -3], [1234, 5678]]) {
  ...
  if (expected !== 2 * lhs + rhs || actual !== expected)
    throw new Error(`multi-result-call differential ${lhs}/${rhs}: AOT=${actual}, interpreter=${expected}`);
}

console.log("strict AOT preserved two fixed results across nested CALL/RETURN and matched five interpreter cases");
