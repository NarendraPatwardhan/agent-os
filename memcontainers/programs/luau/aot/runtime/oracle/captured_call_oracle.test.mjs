...
const aot = await instantiateOracle(
  runfile(process.env.LUAU_AOT_CAPTURED_CALL_ORACLE_WASM, "LUAU_AOT_CAPTURED_CALL_ORACLE_WASM"),
  "strict AOT captured-call oracle",
);
...
const source = readFileSync(
  runfile(process.env.LUAU_AOT_CAPTURED_CALL_ORACLE_SOURCE, "LUAU_AOT_CAPTURED_CALL_ORACLE_SOURCE"),
);

const aotInit = aot.exports.mc_luau_aot_v1_captured_call_oracle_init;
const aotRun = aot.exports.mc_luau_aot_v1_captured_call_oracle_run;
...
if ([aotInit, aotRun, interpreterInit, interpreterRun, interpreterSourceBuffer].some((item) => typeof item !== "function"))
  throw new Error("captured-call differential exports are incomplete");
...
for (const [lhs, rhs] of [[-50, 8], [0, 0], [20, 22], [7, -3], [1234, 5678]]) {
  ...
  if (expected !== lhs + rhs || actual !== expected)
    throw new Error(`captured-call differential ${lhs}/${rhs}: AOT=${actual}, interpreter=${expected}`);
}

console.log("strict AOT preserved a captured upvalue across GC, nested CALL/RETURN, and matched five interpreter cases");
