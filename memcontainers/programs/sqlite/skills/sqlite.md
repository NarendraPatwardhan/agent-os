...
Use `db:vectorInfo(name)` to confirm dimensions, partition count, metadata-index health, resident bytes, hot payload bytes, and cold vector bytes. Use `db:vectorHealth(name)` after bulk loads or heavy churn; check graph-quality ratios and the bounded recall estimate, and run `db:vectorRebuild(name)` if it reports graph damage. Use `db:vectorQuantization(name)` when you need to know whether traversal is int8, bit, or f32-rescored.
