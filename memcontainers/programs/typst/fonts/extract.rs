...
//! bytes ride in via `typst_assets`, an `include_bytes!` crate); the only dep. The ustar writer is
//! hand-rolled, mirroring mc-svc-manifest / mc-roster (dependency-free, fixed owner/mtime → reproducible).
//!
...
/// A 512-byte ustar header for a regular file `path` (octal `mode`, owner 0, mtime 0 — fixed for
/// reproducibility). Mirrors mc-svc-manifest's `file_header`.
