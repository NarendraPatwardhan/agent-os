...
#[derive(Deserialize)]
struct Vector { name: String, source: Option<String>, mutation: String, offset: Option<usize>, value: Option<u32>, length: Option<usize>, error: String }
...
fn mutate(mut bytes: Vec<u8>, v: &Vector) -> Vec<u8> {
    match v.mutation.as_str() {
        "u32" => bytes[v.offset.unwrap()..v.offset.unwrap() + 4].copy_from_slice(&v.value.unwrap().to_le_bytes()),
...
fn both_host_families_reject_the_shared_malformed_snapshot_vectors() {
    ...
    let vectors: Vec<Vector> = serde_json::from_slice(&runfile("_main/memcontainers/conformance/snapshot_vectors.json")).unwrap();
    let (sink, _) = CaptureSink::new();
    let mut host = KernelHostBuilder::new(kernel.clone()).with_base_image(Some(image)).with_stdout(Box::new(sink)).deterministic().build().unwrap();
    ...
    for vector in vectors {
        ...
        let bad = mutate(if is_incremental { incremental.clone() } else { valid.clone() }, &vector);
        let builder = KernelHostBuilder::new(kernel.clone()).deterministic();
        let result = if is_incremental { builder.restore_incremental(&bad, &valid) } else { builder.restore(&bad) };
