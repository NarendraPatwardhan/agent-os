...
interface Vector { name: string; source?: "full" | "incremental"; mutation: string; offset?: number; value?: number; length?: number; error: string }
...
function mutate(input: Uint8Array, v: Vector): Uint8Array {
  ...
  switch (v.mutation) {
    case "u32": dv.setUint32(v.offset!, v.value!, true); break;
    case "zero": bytes.fill(0, v.offset!, v.offset! + v.length!); break;
    case "byte": bytes[v.offset!] = v.value!; break;
    case "flip": bytes[v.offset!] ^= 0xff; break;
    case "append": { const next = new Uint8Array(bytes.length + 1); next.set(bytes); bytes = next; break; }
    case "truncate": bytes = bytes.subarray(0, bytes.length - 1); break;
    default: throw new Error(`unknown mutation ${v.mutation}`);
  }
  return bytes;
}
...
const vectors = JSON.parse(new TextDecoder().decode(file(process.env.MC_SNAPSHOT_VECTORS))) as Vector[];
const discard: StreamSink = { write() {} };
const host = await new KernelHostBuilder(wasm).withBaseImage(image).deterministic().withStdout(discard).build();
...
for (const vector of vectors) {
  try {
    ...
    await new KernelHostBuilder(wasm).deterministic().restore(
      bad,
      vector.source === "incremental" ? valid : undefined,
    );
  } catch (error) {
