    ...
    for (;;) {
        ...
        if (rr == -2) {
            if (type == GE_FRAME_RUN) {
                const char *msg = "{\"ok\":false,\"code\":2,\"stdout\":\"\",\"stderr\":\"frame "
                                  "exceeds 17 MiB cap\"}";
                if (ge_frame_write(stdout, GE_FRAME_RUN, msg, strlen(msg)) != 0)
                    break;
            } else {
                uint8_t fail[4] = {28, 0, 0, 0}; /* AgentOS EINVAL */
                if (ge_frame_write(stdout, type, fail, sizeof(fail)) != 0)
                    break;
            }
            continue;
        }
