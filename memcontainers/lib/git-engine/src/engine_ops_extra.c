...
int op_rm(ge_engine *e, const char *args) {
    ...
    const git_index_entry *prior = git_index_get_bypath(index, path, 0);
    if (!prior) {
        git_index_free(index);
        ge_set_err(e, "rm: path is not tracked");
        return -1;
    }
    git_index_entry saved = *prior;
    char *saved_path = strdup(prior->path);
    if (!saved_path) {
        git_index_free(index);
        ge_set_err(e, "rm: out of memory");
        return -1;
    }
    ...
    if (git_index_write(index) != 0) {
        free(saved_path);
        git_index_free(index);
        ge_set_err_git(e, "rm: write index");
        return -1;
    }
    if (unlink(full) != 0 && errno != ENOENT) {
        int restored = git_index_add(index, &saved) == 0 && git_index_write(index) == 0;
        free(saved_path);
        git_index_free(index);
        ge_set_err(e, restored ? "rm: cannot remove worktree file"
                               : "rm: cannot remove worktree file and index rollback failed");
        return -1;
    }
    free(saved_path);
    git_index_free(index);
...
int op_diff(ge_engine *e, const char *args, char **out_owned) {
    ...
    if (args) {
        ...
        if (parr) {
            ...
            char item[1024];
            int array_rc;
            while ((array_rc = jmin_array_next_string(&cur, item, sizeof(item))) == 0) {
                if (!item[0])
                    continue;
                if (!ge_safe_worktree_relpath(item)) {
            ...
            if (array_rc < 0) {
                ge_set_err(e, "diff: paths entries must be strings");
                return -1;
            }
            const char *tail = cur;
            ...
            if (jmin_get_string(args, "path", path, sizeof(path)) == 0 && path[0]) {
                if (!ge_safe_worktree_relpath(path)) {
                    ge_set_err(e, "diff: bad path");
                    return -1;
                }
...
int op_show(ge_engine *e, const char *args, char **out_owned) {
    ...
    if ((size_t)need > GE_OUT_STREAM_MAX_BYTES) {
        git_object_free(obj);
        ge_set_err(e, "show: output exceeds 16 MiB cap");
        return -1;
    }
...
int op_config(ge_engine *e, const char *args, char **out_owned) {
    ...
    if (strcmp(action, "set") == 0) {
        ...
        const char *keys[] = {
            "user.name",          "user.email",        "core.bare", "remote.origin.url",
            "branch.main.remote", "branch.main.merge", NULL,
        };
...
static int sparse_require_clean(ge_engine *e) {
    ...
    for (size_t i = 0; i < n; i++) {
        ...
        if (entry && entry->status == GIT_STATUS_WT_DELETED && entry->index_to_workdir) {
            ...
            sparse_missing = indexed && (indexed->flags_extended & GIT_INDEX_ENTRY_SKIP_WORKTREE);
        }
...
static int sparse_index_set_cone(ge_engine *e, char cones[][256], size_t ncones, int enable) {
    git_index *index = NULL;
    if (git_repository_index(&index, e->repo) != 0) {
 ...
 * libc lstat maps to MEMFS FS.lstat. */
...
static int cone_prune_walk(const char *root, const char *rel, char cones[][256], size_t ncones) {
...
static int cone_prune_walk_at(int dir_fd, const char *rel, char cones[][256], size_t ncones) {
    ...
    *ncones = 0;
    for (char *line = contents, *next; line && *line; line = next) {
        ...
        while (len > 0 && path[len - 1] == '/')
            path[--len] = '\0';
        if (!*path || !ge_safe_worktree_relpath(path))
            return -1;
        if (negate) {
            for (size_t i = 0; i < *ncones; i++) {
...
static int sparse_rollback(ge_engine *e, const char *sc, const char *backup, int had_backup,
                           int sparse_present, int sparse_value, int cone_present, int cone_value,
                           char old_cones[][256], size_t old_ncones) {
    int failed = 0;
...
static int sparse_fail_after_install(ge_engine *e, const char *reason, const char *sc,
                                     const char *backup, int had_backup, int sparse_present,
                                     int sparse_value, int cone_present, int cone_value,
                                     char old_cones[][256], size_t old_ncones) {
    char primary[sizeof(e->err)];
    snprintf(primary, sizeof(primary), "%s",
             reason && *reason ? reason : "sparse operation failed");
    if (sparse_rollback(e, sc, backup, had_backup, sparse_present, sparse_value, cone_present,
                        cone_value, old_cones, old_ncones) != 0) {
...
int op_sparse_set(ge_engine *e, const char *args) {
    ...
    int old_cone = 0;
    if (sparse_capture_config(e, &old_sparse_present, &old_sparse, &old_cone_present, &old_cone) !=
        0)
        return -1;
    char patterns[4096] = "";
    ...
    if (arr) {
        ...
        if (array_rc < 0) {
            ge_set_err(e, "sparse-set: patterns entries must be strings");
            return -1;
        }
        if (!patterns[0]) {
            ge_set_err(e, "sparse-set: need patterns");
            return -1;
        }
    ...
    if (sparse_read_file(sc, old_pattern, sizeof(old_pattern), &old_pattern_present) != 0 ||
        (old_sparse &&
         (!old_pattern_present || sparse_parse_cones(old_pattern, old_cones, &old_ncones) != 0))) {
    ...
    for (char *p = patterns, *n; p && *p; p = n) {
        ...
        if (!*p || !ge_safe_worktree_relpath(p)) {
        ...
        if (negate) {
            ...
            if (sparse_appendf(sparse_file, sizeof(sparse_file), &sparse_file_len, "/%s/\n/%s/**\n",
                               p, p) != 0) {
                ge_set_err(e, "sparse-set: patterns too long");
                return -1;
            }
    ...
    if (sparse_set_config(e, 1, 1) != 0) {
        return sparse_fail_after_install(e, "sparse-set: config failed", sc, backup, had_backup,
                                         old_sparse_present, old_sparse, old_cone_present, old_cone,
    ...
    if (sparse_index_set_cone(e, cones, ncones, 1) != 0) {
        ...
        return sparse_fail_after_install(e, reason, sc, backup, had_backup, old_sparse_present,
                                         old_sparse, old_cone_present, old_cone, old_cones,
                                         old_ncones);
    }
    if (sparse_checkout_head(e) != 0) {
    ...
    if (cone_prune_walk(e->root, "", cones, ncones) != 0) {
        return sparse_fail_after_install(e, "sparse-set: prune failed", sc, backup, had_backup,
                                         old_sparse_present, old_sparse, old_cone_present, old_cone,
                                         old_cones, old_ncones);
    }
    if (sparse_remove_file(backup) != 0) {
        return sparse_fail_after_install(e, "sparse-set: cannot remove metadata backup", sc, backup,
...
int op_sparse_disable(ge_engine *e, const char *args) {
    ...
    if (sparse_remove_file(backup) != 0) {
        return sparse_fail_after_install(e, "sparse-disable: cannot remove metadata backup", sc,
                                         backup, had_backup, old_sparse_present, old_sparse,
                                         old_cone_present, old_cone, old_cones, old_ncones);
    }
    return 0;
}
...
int op_submodule_list(ge_engine *e, char **out_owned) {
    ...
    char path[4096];
    int path_n = snprintf(path, sizeof(path), "%s/.gitmodules", e->root);
    if (path_n < 0 || (size_t)path_n >= sizeof(path)) {
        ge_set_err(e, "submodule: .gitmodules path too long");
        return -1;
    }
    FILE *f = fopen(path, "rb");
    if (!f) {
        if (errno != ENOENT) {
            ge_set_err(e, "submodule: cannot open .gitmodules");
            return -1;
        }
    ...
    if (read_failed || close_failed) {
        ge_set_err(e, "submodule: cannot read .gitmodules");
        return -1;
    }
    if (n >= sizeof(raw) - 1) {
    ...
    while (line && *line) {
        ...
        if (line[0] == '#' || line[0] == ';' || line[0] == '\0') {
            ...
            if (in_sub && cur_name[0]) {
                ...
                if (ge_submodule_append_entry(&out, &used, &cap, &first, cur_name, cur_path,
                                              cur_url, gl[0] ? gl : NULL) != 0) {
                    free(out);
                    ge_set_err(e, "submodule: out of memory");
                    return -1;
                }
            ...
            if (strncmp(line, "[submodule ", 11) == 0) {
                ...
                if (!in_sub) {
                    free(out);
                    ge_set_err(e, "submodule: malformed or oversized section name");
                    return -1;
                }
            }
        } else if (in_sub) {
            ...
            if (eq) {
                *eq = '\0';
                ...
                if (strcmp(key, "path") == 0) {
                    if (strlen(val) >= sizeof(cur_path)) {
                        free(out);
                        ge_set_err(e, "submodule: path is too long");
                        return -1;
                    }
                    snprintf(cur_path, sizeof(cur_path), "%s", val);
                } else if (strcmp(key, "url") == 0) {
                    if (strlen(val) >= sizeof(cur_url)) {
                        free(out);
                        ge_set_err(e, "submodule: url is too long");
                        return -1;
                    }
