...
int ge_safe_worktree_relpath(const char *path) {
    return ge_safe_relpath(path) && strcmp(path, ".git") != 0 && strncmp(path, ".git/", 5) != 0;
}

/* Growable string builder (status/log stdout). */
static int sb_ensure(char **buf, size_t *cap, size_t need) {
    if (need > GE_OUT_STREAM_MAX_BYTES + 1u)
        return -1;
    if (need <= *cap)
        return 0;
...
void ge_join_path(char *out, size_t cap, const char *root, const char *rel) {
    if (!out || cap == 0)
        return;
    int n = snprintf(out, cap, "%s/%s", root ? root : "", rel ? rel : "");
    if (n < 0 || (size_t)n >= cap)
        out[0] = '\0';
}

int ge_worktree_path(ge_engine *e, const char *rel, int allow_missing, char *out, size_t out_cap) {
    if (!e || !rel || !ge_safe_worktree_relpath(rel) || !out || out_cap == 0) {
...
int ge_worktree_mkdir_parents(ge_engine *e, const char *rel) {
    if (!e || !rel || !ge_safe_worktree_relpath(rel)) {
        ge_set_err(e, "path: invalid worktree-relative path");
        return -1;
    }
...
static int valid_client_token(const char *token) {
...
static int set_client_token_from_args(ge_engine *e, const char *args) {
    clear_client_token(e);
    if (!e || !args || !jmin_has_member(args, "client_token"))
        return 0;
    if (jmin_get_string(args, "client_token", e->client_token, sizeof(e->client_token)) != 0 ||
        !valid_client_token(e->client_token)) {
        clear_client_token(e);
        return -1;
    }
    return 0;
    ...
    if (result_key) {
        ...
        if (*val == '{') {
            ...
            if (out) {
                ...
                int n = snprintf(out, need, "%.*s\"client_token\":\"%s\",%s", (int)prefix, resp,
                                 esc, val + 1);
        ...
        out = (char *)malloc(need);
        if (out) {
            int n = snprintf(out, need, "%.*s,\"result\":{\"client_token\":\"%s\"}}",
                             (int)(rlen - 1), resp, esc);
            if (n < 0 || (size_t)n >= need) {
                free(out);
                out = NULL;
...
static char *resp_usage(ge_engine *e, const char *msg) { return resp_err(e, 2, msg); }

static int ensure_real_directory(const char *path, mode_t mode, int create) {
    struct stat st;
    if (lstat(path, &st) != 0) {
        if (!create || errno != ENOENT || mkdir(path, mode) != 0 || lstat(path, &st) != 0)
            return -1;
    }
    return !S_ISLNK(st.st_mode) && S_ISDIR(st.st_mode) ? 0 : -1;
}

/* Ensure worktree /.git/mc/out/ exists without following metadata symlinks. */
static int ensure_out_dir(ge_engine *e) {
    if (!e)
        return -1;
    char p[4096];
    int n = snprintf(p, sizeof(p), "%s/.git", e->root);
    if (n < 0 || (size_t)n >= sizeof(p) || ensure_real_directory(p, 0755, 0) != 0)
        return -1;
    n = snprintf(p, sizeof(p), "%s/.git/mc", e->root);
    if (n < 0 || (size_t)n >= sizeof(p) || ensure_real_directory(p, 0755, 1) != 0)
        return -1;
    n = snprintf(p, sizeof(p), "%s/.git/mc/out", e->root);
    return n >= 0 && (size_t)n < sizeof(p) && ensure_real_directory(p, 0755, 1) == 0 ? 0 : -1;
}

/* Write a bounded full stdout body under the request-specific stream path. */
static int write_out_stream(ge_engine *e, const char *body, size_t n, char *rel_out,
                            size_t rel_cap) {
    if (!e || !body)
        return -1;
    if (n > GE_OUT_STREAM_MAX_BYTES)
        return -2;
    if (!e->client_token[0])
        return -3;
    ...
    char p[4096];
    char rel[320];
    int rn = snprintf(rel, sizeof(rel), ".git/mc/out/%s", e->client_token);
    if (rn < 0 || (size_t)rn >= sizeof(rel))
        return -1;
    int pn = snprintf(p, sizeof(p), "%s/%s", e->root, rel);
    if (pn < 0 || (size_t)pn >= sizeof(p))
        return -1;
    char temporary[4096];
    int tn = snprintf(temporary, sizeof(temporary), "%s.tmp-%ld-%llu", p, (long)getpid(),
                      (unsigned long long)++e->file_generation);
    ...
    size_t wrote = 0;
    while (wrote < n) {
        ssize_t nw = write(fd, body + wrote, n - wrote);
        if (nw < 0 && errno == EINTR)
            continue;
        if (nw <= 0) {
            close(fd);
    ...
    if (close(fd) != 0 || rename(temporary, p) != 0) {
        unlink(temporary);
        return -1;
    }
    if (rel_out && rel_cap > 0) {
        int rn = snprintf(rel_out, rel_cap, "%s", rel);
        if (rn < 0 || (size_t)rn >= rel_cap)
            return -1;
    }
    return 0;
}

/* Drop a stale body for the current request stream path. */
static void clear_out_stream(ge_engine *e) {
    if (!e || !e->client_token[0])
        return;
    char p[4096];
    snprintf(p, sizeof(p), "%s/.git/mc/out/%s", e->root, e->client_token);
    unlink(p);
}

void ge_remove_token_stream(ge_engine *e, const char *token) {
    if (!e || !valid_client_token(token))
        return;
    char p[4096];
    int n = snprintf(p, sizeof(p), "%s/.git/mc/out/%s", e->root, token);
 ...
 * When body exceeds the embed limit, stdout contains a preview and
 * result.stream_path names the complete body. */
char *ge_resp_ok_stdout(ge_engine *e, char *stdout_owned, int free_stdout,
    ...
    /* Overflow: a successful envelope always names a complete bounded body. */
    char stream_path[320];
    int stream_rc = e ? write_out_stream(e, body, n, stream_path, sizeof(stream_path)) : -1;
    if (stream_rc != 0) {
        if (free_stdout)
            free(stdout_owned);
        return resp_err(e, 1,
                        stream_rc == -2   ? "stdout exceeds 16 MiB stream cap"
                        : stream_rc == -3 ? "large stdout requires args.client_token"
                                          : "cannot persist complete stdout stream");
    }
    ...
    char base[384];
    int blen = snprintf(base, sizeof(base),
                        "\"truncated\":true,\"stream_path\":\"%s\",\"stdout_bytes\":%zu,"
                        "\"stdout_embed_bytes\":%zu,\"stream_bytes\":%zu",
                        stream_path, n, limit, n);
    ...
    if (extra_result_json && extra_result_json[0] == '{') {
        ...
        if (elen >= 2 && extra_result_json[elen - 1] == '}') {
            ...
            int m = snprintf(result_buf, sizeof(result_buf), "{%s,%s", base, extra_result_json + 1);
...
const char *ge_worktree_root(const ge_engine *e) { return e ? e->root : NULL; }

const char *ge_last_error(const ge_engine *e) { return e ? e->err : "null engine"; }

int ge_response_is_static(const void *p) { return p == (const void *)ge_static_oom; }
...
ge_engine *ge_open(const char *worktree_root) {
    ...
    if (worktree_root[0] != '/')
        return NULL;
    size_t root_len = strlen(worktree_root);
    while (root_len > 1 && worktree_root[root_len - 1] == '/')
        root_len--;
    if (root_len == 1 || root_len > GE_ROOT_MAX_BYTES)
        return NULL;
    char normalized[4096];
    memcpy(normalized, worktree_root, root_len);
    normalized[root_len] = '\0';
    char resolved[4096];
    if (!realpath(normalized, resolved) || strcmp(normalized, resolved) != 0)
        return NULL; /* reject symlinks, dot segments, and non-canonical roots */
    struct stat st;
    if (lstat(normalized, &st) != 0 || !S_ISDIR(st.st_mode) || S_ISLNK(st.st_mode))
        return NULL;
    ge_engine *e = (ge_engine *)calloc(1, sizeof(*e));
    if (!e)
        return NULL;
    snprintf(e->root, sizeof(e->root), "%s", normalized);
    git_libgit2_init();
    ...
    if (git_repository_open_ext(&e->repo, e->root, GIT_REPOSITORY_OPEN_NO_SEARCH, NULL) != 0)
        e->repo = NULL;
    return e;
}
...
void ge_close(ge_engine *e) {
    ...
    for (size_t i = 0; i < sizeof(e->ctl_responses) / sizeof(e->ctl_responses[0]); i++) {
        ge_remove_token_stream(e, e->ctl_responses[i].token);
        free(e->ctl_responses[i].response);
    }
    char transient[4096];
    int n = snprintf(transient, sizeof(transient), "%s/.git/mc/out", e->root);
    if (n >= 0 && (size_t)n < sizeof(transient)) {
        DIR *out = opendir(transient);
        if (out) {
            struct dirent *entry;
            while ((entry = readdir(out)) != NULL) {
                if (!valid_client_token(entry->d_name))
                    continue;
                char path[4096];
                int pn = snprintf(path, sizeof(path), "%s/%s", transient, entry->d_name);
    ...
    if (n >= 0 && (size_t)n < sizeof(transient))
        rmdir(transient);
    n = snprintf(transient, sizeof(transient), "%s/.git/mc", e->root);
    if (n >= 0 && (size_t)n < sizeof(transient))
        rmdir(transient);
    n = snprintf(transient, sizeof(transient), "%s/.git/agentos/push.pack", e->root);
    if (n >= 0 && (size_t)n < sizeof(transient))
        unlink(transient);
    n = snprintf(transient, sizeof(transient), "%s/.git/agentos", e->root);
    if (n >= 0 && (size_t)n < sizeof(transient))
        rmdir(transient);
...
static int op_write(ge_engine *e, const char *args) {
    char path[1024];
    if (jmin_get_string(args, "path", path, sizeof(path)) != 0 || !ge_safe_worktree_relpath(path)) {
    ...
    mode_t mode = 0644;
    {
        ...
        if (lstat(full, &st) == 0) {
            ...
            free(content);
            set_err(e, "write: cannot inspect destination");
            return -1;
        }
    }

    char temporary[4096];
    int temporary_n = snprintf(temporary, sizeof(temporary), "%s.agentos-write-%ld-%llu", full,
                               (long)getpid(), (unsigned long long)++e->file_generation);
    if (temporary_n < 0 || (size_t)temporary_n >= sizeof(temporary)) {
        free(content);
        set_err(e, "write: temporary path is too long");
        return -1;
    }

    int fd = open(temporary, O_WRONLY | O_CREAT | O_EXCL | O_NOFOLLOW | O_CLOEXEC, mode);
    if (fd < 0 || fchmod(fd, mode) != 0) {
        if (fd >= 0)
            close(fd);
        unlink(temporary);
        free(content);
        set_err(e, "write: cannot create temporary file");
        return -1;
    }
    size_t wrote = 0;
    while (wrote < content_len) {
        ssize_t n = write(fd, content + wrote, content_len - wrote);
        if (n < 0 && errno == EINTR)
            continue;
        if (n <= 0) {
            close(fd);
    ...
    if (close(fd) != 0 || rename(temporary, full) != 0) {
        unlink(temporary);
        free(content);
        set_err(e, "write: cannot atomically replace destination");
        return -1;
    }
    free(content);
    return 0;
...
static int index_add_file(ge_engine *e, git_index *index, const char *rel) {
    if (!ge_safe_worktree_relpath(rel)) {
    ...
    if (st.st_size < 0 || (uint64_t)st.st_size > GE_WRITE_MAX_BYTES) {
        set_err(e, "add: file exceeds 16 MiB staging cap");
        return -1;
    }
    ...
    if ((uint64_t)sz > GE_WRITE_MAX_BYTES) {
        fclose(f);
        set_err(e, "add: file exceeds 16 MiB staging cap");
        return -1;
    }
...
static int walk_add(ge_engine *e, git_index *index, const char *rel) {
    ...
    while ((de = readdir(d)) != NULL) {
        ...
        if (!child_full[0]) {
            set_err(e, "add: worktree path too long");
            rc = -1;
            break;
        }
        struct stat st;
        ...
        /* Bulk add never follows or asks libgit2 to inspect symlinks/specials. */
...
static int stage_deletions(ge_engine *e, git_index *index) {
    ...
    for (size_t i = n; i > 0; i--) {
        ...
        if (!full[0]) {
            set_err(e, "add: tracked worktree path too long");
            return -1;
        }
        struct stat st;
        if (lstat(full, &st) != 0) {
            if (errno != ENOENT && errno != ENOTDIR) {
                set_err(e, "add: cannot inspect tracked worktree path");
                return -1;
            }
...
static int op_log(ge_engine *e, const char *args, char **out_owned, char *result_buf,
                  size_t result_cap) {
    if (out_owned)
        *out_owned = NULL;
    ...
    if (result_buf && result_cap > 0) {
        snprintf(result_buf, result_cap, "{\"count\":%lld,\"max_count\":%lld,\"bounded\":%s%s}",
                 (long long)count, (long long)max_count, bounded ? "true" : "false",
                 more ? ",\"more\":true" : "");
    }
    *out_owned = buf;
    return 0;
...
void ge_import_pack_abort(ge_engine *e) {
    if (!e)
        return;
    import_pack_reset(e);
    e->err[0] = '\0';
...
static int pack_scan_oid_array(const char *arr, git_oid *tips, size_t max, size_t *n,
                               ge_engine *e) {
    const char *cur = pack_skip_ws(arr);
    if (!cur || *cur != '[') {
    ...
    for (;;) {
        char hex[GIT_OID_HEXSZ + 1];
        if (jmin_array_next_string(&cur, hex, sizeof(hex)) != 0)
            break;
        git_oid oid;
    ...
    const char *end = pack_skip_ws(cur);
    if (*end != ']') {
        set_err(e, "pack.build: oids entries must be 40-hex strings");
        return -1;
    }
    return 0;
}
...
static int pack_scan_command_hashes(const char *region, git_oid *tips, size_t max, size_t *n,
                                    ge_engine *e) {
    if (!region)
        return 0;
    ...
    while (jmin_array_next_object(&cur, &obj) == 0) {
        char hex[GIT_OID_HEXSZ + 1] = "";
        if (jmin_obj_get_string(obj, "newHash", hex, sizeof(hex)) != 0)
            (void)jmin_obj_get_string(obj, "hash", hex, sizeof(hex));
        if (!hex[0]) {
            set_err(e, "pack.build: command is missing newHash/hash");
            return -1;
        }
        git_oid oid;
        if (git_oid_fromstr(&oid, hex) != 0) {
            set_err(e, "pack.build: bad command oid");
            return -1;
        }
        if (pack_oid_add(tips, n, max, &oid) != 0) {
            set_err(e, "pack.build: too many oids");
    ...
    if (*pack_skip_ws(cur) != ']') {
        set_err(e, "pack.build: commands entries must be objects");
        return -1;
    }
    return 0;
}
    ...
    *n = 0;
    ...
    if (json && *json) {
        if (*json == '[') {
            ...
            const char *oids_v = jmin_get_array(json, "oids");
            if (oids_v && *oids_v == '[') {
                explicit = 1;
                if (pack_scan_oid_array(oids_v, tips, max, n, e) != 0)
                    return -1;
            }
            const char *cmds = jmin_get_array(json, "commands");
            if (cmds) {
                explicit = 1;
...
static int pack_collect_haves(ge_engine *e, const char *oids_json, git_oid *haves, size_t max,
    ...
    *n = 0;
    ...
    if (!json || *json != '{')
        return 0;
    const char *haves_v = jmin_get_array(json, "haves");
    if (!haves_v || *haves_v != '[')
        return 0;
...
static int bounded_pack_append(void *buf, size_t size, void *payload) {
    bounded_pack_output *out = (bounded_pack_output *)payload;
    if (!out || size > GE_PACK_MAX_BYTES || out->len > GE_PACK_MAX_BYTES - size) {
        if (out)
            out->exceeded = 1;
        return -1;
    }
    size_t need = out->len + size;
    if (need > out->cap) {
        size_t cap = out->cap ? out->cap : 64u * 1024u;
        while (cap < need) {
            if (cap >= GE_PACK_MAX_BYTES / 2) {
                cap = GE_PACK_MAX_BYTES;
                break;
            }
            cap *= 2;
        }
        uint8_t *next = (uint8_t *)realloc(out->bytes, cap);
        if (!next)
            return -1;
        out->bytes = next;
        out->cap = cap;
    }
    memcpy(out->bytes + out->len, buf, size);
    out->len += size;
    return 0;
}
...
static int pack_build_core(ge_engine *e, const char *oids_json, uint8_t **out, size_t *out_len,
    ...
    bounded_pack_output built = {0};
    if (git_packbuilder_foreach(pb, bounded_pack_append, &built) != 0) {
        set_err(e, built.exceeded ? "pack.build: exceeds 64 MiB cap"
                                  : "pack.build: bounded writer failed");
        git_packbuilder_free(pb);
        free(built.bytes);
        return -1;
    }
    ...
    if (!built.bytes || built.len < 4 || memcmp(built.bytes, "PACK", 4) != 0) {
...
int ge_pack_build(ge_engine *e, const char *oids_json, uint8_t **out, size_t *out_len) {
    if (oids_json && oids_json[0] && jmin_validate_document(oids_json) != 0) {
        if (e)
            set_err(e, "pack.build: invalid JSON");
        return -1;
    }
...
static int op_pack_build(ge_engine *e, const char *args, char *result_json, size_t result_cap) {
    char stale[4096];
    int stale_n = snprintf(stale, sizeof(stale), "%s/.git/agentos/push.pack", e->root);
    if (stale_n < 0 || (size_t)stale_n >= sizeof(stale)) {
        set_err(e, "pack.build: export path overflow");
        return -1;
    }
    unlink(stale);
    ...
    int gitdir_n = snprintf(gitdir, sizeof(gitdir), "%s/.git", e->root);
    int adir_n = snprintf(adir, sizeof(adir), "%s/.git/agentos", e->root);
    int full_n = snprintf(full, sizeof(full), "%s/.git/agentos/push.pack", e->root);
    if (gitdir_n < 0 || (size_t)gitdir_n >= sizeof(gitdir) || adir_n < 0 ||
        (size_t)adir_n >= sizeof(adir) || full_n < 0 || (size_t)full_n >= sizeof(full) ||
        ensure_real_directory(gitdir, 0755, 0) != 0 || ensure_real_directory(adir, 0755, 1) != 0) {
        free(pack);
        set_err(e, "pack.build: unsafe export directory");
        return -1;
    }

    char temporary[4096];
    int temporary_n = snprintf(temporary, sizeof(temporary), "%s.tmp-%ld-%llu", full,
                               (long)getpid(), (unsigned long long)++e->file_generation);
    int fd = temporary_n >= 0 && (size_t)temporary_n < sizeof(temporary)
                 ? open(temporary, O_WRONLY | O_CREAT | O_EXCL | O_NOFOLLOW | O_CLOEXEC, 0600)
                 : -1;
    if (fd < 0) {
        free(pack);
        set_err(e, "pack.build: cannot create export file");
        return -1;
    }
    size_t wrote = 0;
    while (wrote < pack_len) {
        ssize_t nw = write(fd, pack + wrote, pack_len - wrote);
        if (nw < 0 && errno == EINTR)
            continue;
        if (nw <= 0)
            break;
    ...
    if (close(fd) != 0 || wrote != pack_len || rename(temporary, full) != 0) {
        free(pack);
        unlink(temporary);
        unlink(full);
...
typedef struct {
    ...
    int was_symbolic;
    git_oid prior_oid;
    char prior_symbolic[256];
} refs_import_item;

static int refs_import_validate_one(ge_engine *e, refs_import_item *items, size_t count,
    ...
    int valid = 0;
    if (git_reference_name_is_valid(&valid, name) != 0 || !valid) {
        set_err(e, "refs.import: invalid ref name");
        return -1;
    }
    for (size_t i = 0; i < count; i++) {
        if (strcmp(items[i].name, name) == 0) {
            set_err(e, "refs.import: duplicate ref name");
            return -1;
        }
    }
    refs_import_item *item = &items[count];
    memset(item, 0, sizeof(*item));
    snprintf(item->name, sizeof(item->name), "%s", name);
    if (git_oid_fromstr(&item->next_oid, hash) != 0) {
        set_err(e, "refs.import: bad hash");
        return -1;
    }
    git_object *obj = NULL;
    if (git_object_lookup(&obj, e->repo, &item->next_oid, GIT_OBJECT_ANY) != 0) {
        set_err(e, "refs.import: target object missing");
        return -1;
    }
    git_object_free(obj);

    git_reference *prior = NULL;
    int lookup_rc = git_reference_lookup(&prior, e->repo, name);
    if (lookup_rc == 0) {
        item->existed = 1;
        if (git_reference_type(prior) == GIT_REFERENCE_SYMBOLIC) {
            const char *target = git_reference_symbolic_target(prior);
            if (!target || strlen(target) >= sizeof(item->prior_symbolic)) {
                git_reference_free(prior);
                set_err(e, "refs.import: prior symbolic ref is too long");
                return -1;
            }
            item->was_symbolic = 1;
            snprintf(item->prior_symbolic, sizeof(item->prior_symbolic), "%s", target);
        } else {
            const git_oid *oid = git_reference_target(prior);
            if (!oid) {
                git_reference_free(prior);
                set_err(e, "refs.import: prior direct ref has no target");
                return -1;
            }
            git_oid_cpy(&item->prior_oid, oid);
        }
        git_reference_free(prior);
    } else if (lookup_rc != GIT_ENOTFOUND) {
        set_err_git(e, "refs.import: cannot inspect prior ref");
        return -1;
    }
    return 0;
}

static int refs_import_restore(ge_engine *e, refs_import_item *items, size_t count) {
    git_transaction *tx = NULL;
    if (git_transaction_new(&tx, e->repo) != 0)
        return -1;
    for (size_t i = 0; i < count; i++) {
        if (git_transaction_lock_ref(tx, items[i].name) != 0)
            goto fail;
    }
    for (size_t i = 0; i < count; i++) {
        git_reference *current = NULL;
        int lookup_rc = git_reference_lookup(&current, e->repo, items[i].name);
        int rc;
        if (!items[i].existed) {
            if (lookup_rc == GIT_ENOTFOUND)
                continue;
            const git_oid *target =
                lookup_rc == 0 && git_reference_type(current) == GIT_REFERENCE_DIRECT
                    ? git_reference_target(current)
                    : NULL;
            if (!target || !git_oid_equal(target, &items[i].next_oid)) {
                git_reference_free(current);
                goto fail;
            }
            git_reference_free(current);
            rc = git_transaction_remove(tx, items[i].name);
        } else {
            if (lookup_rc != 0) {
                git_reference_free(current);
                goto fail;
            ...
            int already_prior = 0;
            if (items[i].was_symbolic && git_reference_type(current) == GIT_REFERENCE_SYMBOLIC) {
                const char *target = git_reference_symbolic_target(current);
                already_prior = target && strcmp(target, items[i].prior_symbolic) == 0;
            } else if (!items[i].was_symbolic &&
                       git_reference_type(current) == GIT_REFERENCE_DIRECT) {
                const git_oid *target = git_reference_target(current);
                already_prior = target && git_oid_equal(target, &items[i].prior_oid);
            }
            if (already_prior) {
                git_reference_free(current);
                continue;
            }
            const git_oid *applied = git_reference_type(current) == GIT_REFERENCE_DIRECT
                                         ? git_reference_target(current)
                                         : NULL;
            if (!applied || !git_oid_equal(applied, &items[i].next_oid)) {
                git_reference_free(current);
                goto fail; /* never overwrite a concurrent third value */
            }
            git_reference_free(current);
            if (items[i].was_symbolic)
                rc = git_transaction_set_symbolic_target(tx, items[i].name, items[i].prior_symbolic,
                                                         NULL, NULL);
            else
                rc = git_transaction_set_target(tx, items[i].name, &items[i].prior_oid, NULL, NULL);
        }
        if (rc != 0)
            goto fail;
    }
    if (git_transaction_commit(tx) != 0)
        goto fail;
...
static int refs_import_apply(ge_engine *e, refs_import_item *items, size_t count) {
    git_transaction *tx = NULL;
    if (git_transaction_new(&tx, e->repo) != 0) {
        set_err_git(e, "refs.import: transaction");
        return -1;
    }
    /* Acquire every ref lock before queueing or applying any update. */
    for (size_t i = 0; i < count; i++) {
        if (git_transaction_lock_ref(tx, items[i].name) != 0) {
            set_err_git(e, "refs.import: lock");
            git_transaction_free(tx);
            return -1;
        }
    }
    for (size_t i = 0; i < count; i++) {
        if (git_transaction_set_target(tx, items[i].name, &items[i].next_oid, NULL, NULL) != 0) {
            set_err_git(e, "refs.import: queue update");
            git_transaction_free(tx);
            return -1;
        }
    }
    if (git_transaction_commit(tx) != 0) {
        char original[sizeof(e->err)];
        set_err_git(e, "refs.import: commit");
        snprintf(original, sizeof(original), "%s", e->err);
        git_transaction_free(tx);
        if (refs_import_restore(e, items, count) != 0)
            snprintf(e->err, sizeof(e->err), "refs.import: commit failed and rollback failed");
        else
            snprintf(e->err, sizeof(e->err), "%s", original);
        return -1;
    }
    git_transaction_free(tx);
    return 0;
...
/* Single {name,hash} or args.refs array. Validation captures every prior value;
 * a failed transaction commit attempts to restore the captured ref set. */
static int op_refs_import(ge_engine *e, const char *args) {
    if (ensure_repo(e) != 0)
        return -1;

    refs_import_item *items =
        (refs_import_item *)calloc(GE_REFS_IMPORT_MAX, sizeof(refs_import_item));
    if (!items) {
        set_err(e, "refs.import: out of memory");
        return -1;
    }
    size_t count = 0;
    ...
    if (arr) {
        ...
        const char *obj = NULL;
        while (jmin_array_next_object(&cur, &obj) == 0) {
            if (count >= GE_REFS_IMPORT_MAX) {
                set_err(e, "refs.import: too many refs");
                free(items);
                return -1;
            }
            char name[256];
            ...
            if (refs_import_validate_one(e, items, count, name, hash) != 0) {
                free(items);
                return -1;
            }
            count++;
        ...
        int rc = refs_import_apply(e, items, count);
        free(items);
        return rc;
    }
    ...
    if (refs_import_validate_one(e, items, 0, name, hash) != 0) {
        free(items);
        return -1;
    }
    int rc = refs_import_apply(e, items, 1);
    free(items);
    return rc;
}
...
static int op_fetch_apply(ge_engine *e, const char *args) {
    ...
    if (remote_len > 127 || strcmp(remote, ".") == 0 || strcmp(remote, "..") == 0) {
        set_err(e, "fetch.apply: bad remote name");
        return -1;
    }
    for (size_t i = 0; i < remote_len; i++) {
        unsigned char c = (unsigned char)remote[i];
    ...
    if (short_branch && *short_branch) {
        ...
        if (tracking_n < 0 || (size_t)tracking_n >= sizeof(tracking)) {
            set_err(e, "fetch.apply: remote-tracking ref is too long");
            return -1;
        }
    ...
    /* FETCH_HEAD file (not a symbolic ref) — minimal atomic single-line form. */
    {
        char path[4096];
        int path_n = snprintf(path, sizeof(path), "%s/.git/FETCH_HEAD", e->root);
        if (path_n < 0 || (size_t)path_n >= sizeof(path)) {
            set_err(e, "fetch.apply: FETCH_HEAD path is too long");
            return -1;
        }
        char body[1024];
        int body_n = snprintf(body, sizeof(body), "%s\t\tbranch '%s' of remote '%s'\n", hash,
                              short_branch && *short_branch ? short_branch : name, remote);
        if (body_n < 0 || (size_t)body_n >= sizeof(body)) {
            set_err(e, "fetch.apply: FETCH_HEAD entry is too long");
            return -1;
        }

        char temporary[4096];
        int temporary_n = snprintf(temporary, sizeof(temporary), "%s.agentos-fetch-%ld-%llu", path,
                                   (long)getpid(), (unsigned long long)++e->file_generation);
        if (temporary_n < 0 || (size_t)temporary_n >= sizeof(temporary)) {
            set_err(e, "fetch.apply: temporary path is too long");
            return -1;
        }
        int fd = open(temporary, O_WRONLY | O_CREAT | O_EXCL | O_NOFOLLOW | O_CLOEXEC, 0600);
        if (fd < 0) {
            set_err(e, "fetch.apply: cannot create temporary FETCH_HEAD");
            return -1;
        }
        size_t wrote = 0;
        while (wrote < (size_t)body_n) {
            ssize_t n = write(fd, body + wrote, (size_t)body_n - wrote);
            if (n < 0 && errno == EINTR)
                continue;
            if (n <= 0) {
                close(fd);
                unlink(temporary);
                set_err(e, "fetch.apply: cannot write FETCH_HEAD");
                return -1;
            }
            wrote += (size_t)n;
        }
        if (close(fd) != 0 || rename(temporary, path) != 0) {
            unlink(temporary);
            set_err(e, "fetch.apply: cannot atomically replace FETCH_HEAD");
            return -1;
        }
    }
    return 0;
...
int ge_validate_request_json(const char *request_json) {
    if (!request_json || strlen(request_json) > GE_REQUEST_MAX_BYTES)
        return -1;
    char op[64];
    const char *args = NULL;
    return jmin_validate_request(request_json, op, sizeof(op), &args);
}
...
char *ge_run_json(ge_engine *e, const char *request_json) {
    ...
    if (strlen(request_json) > GE_REQUEST_MAX_BYTES)
        return resp_err(e, 2, "request exceeds 1 MiB cap");
    if (ge_validate_request_json(request_json) != 0)
        return resp_usage(e, "invalid request: expected exactly {op:string,args?:object}");

    char op[64];
    const char *args = NULL;
    if (jmin_validate_request(request_json, op, sizeof(op), &args) != 0)
        return resp_usage(e, "invalid request");
    ...
    if (set_client_token_from_args(e, args) != 0)
        return resp_usage(e, "invalid args.client_token");

    /* ── Fail closed: product network dials (engine never dials) ──────────── */
    if (strcmp(op, "clone") == 0 || strcmp(op, "fetch") == 0 || strcmp(op, "pull") == 0 ||
        strcmp(op, "push") == 0) {
        return resp_err(e, 1,
                        "git: use host-mediated remotes (pack.import / *.apply); "
