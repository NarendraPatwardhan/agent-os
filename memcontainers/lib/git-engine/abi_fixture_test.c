...
static int expect_fail(ge_engine *e, const char *req, const char *needle) {
    ...
    int ok =
        (strstr(resp, "\"ok\":false") != NULL) && (needle == NULL || strstr(resp, needle) != NULL);
    if (!ok)
...
static int test_add_all(ge_engine *e) {
    if (expect_ok(e, "{\"op\":\"write\",\"args\":{\"path\":\"all_a.txt\",\"content\":\"a\\n\"}}"))
        return 1;
    if (expect_ok(e,
                  "{\"op\":\"write\",\"args\":{\"path\":\"sub/all_b.txt\",\"content\":\"b\\n\"}}"))
        return 1;
    ...
    int bad = (strstr(resp, "\"ok\":true") == NULL) || (strstr(resp, "all_a.txt") == NULL) ||
...
static int test_pack_build(ge_engine *e) {
    ...
    {
        char req[160];
        snprintf(req, sizeof(req), "{\"op\":\"pack.build\",\"args\":{\"oids\":[\"%s\"]}}", hex);
        char *resp = ge_run_json(e, req);
    ...
    /* second commit + pack with haves=[parent] must be smaller than full tip pack. */
    {
        ...
        char thin_json[160];
        snprintf(thin_json, sizeof(thin_json), "{\"oids\":[\"%s\"],\"haves\":[\"%s\"]}", hex2, hex);
        uint8_t *thin = NULL;
        size_t thin_len = 0;
...
static int test_pack_cap(ge_engine *e) {
    ...
    if (rc1 != 0) {
        fprintf(stderr,
                "note: first pack append failed (%s); multi-chunk size path "
...
static int test_path_safety(ge_engine *e) {
    ...
    if (expect_fail(e, "{\"op\":\"write\",\"args\":{\"path\":\"a/../b\",\"content\":\"n\"}}",
                    NULL)) {
...
static int test_diff_patch(ge_engine *e) {
    ...
    if (!resp || strstr(resp, "\"ok\":true") == NULL || strstr(resp, "hello.txt") == NULL ||
        strstr(resp, "other.txt") == NULL) {
...
static int test_add_all_deletion(ge_engine *e) {
    if (expect_ok(e,
                  "{\"op\":\"write\",\"args\":{\"path\":\"doomed.txt\",\"content\":\"bye\\n\"}}"))
        return 1;
...
static int test_refs_import_array(ge_engine *e) {
    ...
    snprintf(req, sizeof(req),
             "{\"op\":\"refs.import\",\"args\":{\"refs\":["
             "{\"name\":\"refs/heads/rollback-a\",\"hash\":\"%s\"},"
             "{\"name\":\"bad ref name\",\"hash\":\"%s\"}"
             "]}}",
             hex, hex);
    if (expect_fail(e, req, "invalid ref name"))
        return 1;
    if (expect_fail(e, "{\"op\":\"rev-parse\",\"args\":{\"rev\":\"refs/heads/rollback-a\"}}",
                    "rev-parse") != 0) {
        fprintf(stderr, "refs.import validation leaked the first ref\n");
        return 1;
    }
    return 0;
}

/* Large stdout requires a request token and writes only its scoped stream. */
static int test_truncated_stdout(ge_engine *e) {
    ...
    if (expect_fail(e, "{\"op\":\"diff\",\"args\":{\"path\":\"trunc.txt\"}}",
                    "large stdout requires args.client_token")) {
        ge_test_set_stdout_max_bytes(0);
        return 1;
    }
    char *resp = ge_run_json(
        e, "{\"op\":\"diff\",\"args\":{\"path\":\"trunc.txt\",\"client_token\":\"abi-trunc\"}}");
    ge_test_set_stdout_max_bytes(0);
    ...
    if (strstr(resp, "\"stream_path\":\".git/mc/out/abi-trunc\"") == NULL) {
        fprintf(stderr, "expected token-scoped result.stream_path:\n%s\n", resp);
        ge_free(resp);
        return 1;
    }
    ...
    char stream[4096];
    snprintf(stream, sizeof(stream), "%s/.git/mc/out/abi-trunc", ge_worktree_root(e));
    FILE *f = fopen(stream, "rb");
    if (!f) {
        fprintf(stderr, "missing token-scoped stream after truncation: %s\n", stream);
        return 1;
    }
    char buf[256];
    ...
    if (n == 0 || (strstr(buf, "diff") == NULL && strstr(buf, "trunc") == NULL)) {
        fprintf(stderr, "token-scoped stream empty or unexpected:\n%s\n", buf);
        return 1;
    }
    return 0;
}
...
static int test_symlink_fail_closed(ge_engine *e) {
    ...
    if (expect_fail(e, "{\"op\":\"add\",\"args\":{\"path\":\"symlink_link.txt\"}}", "symlink"))
        return 1;
    if (expect_fail(e,
    ...
    if (expect_fail(e,
        ...
        expect_fail(e, "{\"op\":\"add\",\"args\":{\"path\":\"symlink_dir/secret.txt\"}}",
                    "symlink") ||
        expect_fail(e, "{\"op\":\"rm\",\"args\":{\"path\":\"symlink_dir/secret.txt\"}}", "symlink"))
        return 1;
...
static int response_or_stream_contains(ge_engine *e, const char *resp, const char *token,
                                       const char *needle) {
    if (resp && strstr(resp, needle) != NULL)
        return 1;
    if (!resp || strstr(resp, "\"truncated\":true") == NULL)
        return 0;

    char path[4096];
    snprintf(path, sizeof(path), "%s/.git/mc/out/%s", ge_worktree_root(e), token);
    FILE *f = fopen(path, "rb");
    if (!f)
        return 0;
    if (fseek(f, 0, SEEK_END) != 0) {
    ...
    fclose(f);
    body[n] = '\0';
    int found = n == (size_t)size && strstr(body, needle) != NULL;
    free(body);
    return found;
}
...
static int test_large_ref_outputs(ge_engine *e) {
    ...
    for (int i = 0; i < 180; i++) {
        ...
        snprintf(req, sizeof(req), "{\"op\":\"branch\",\"args\":{\"name\":\"%s\"}}", last_branch);
        if (expect_ok(e, req))
            return 1;
    }

    const char *token = "abi-large-refs";
    char *resp =
        ge_run_json(e, "{\"op\":\"branch\",\"args\":{\"client_token\":\"abi-large-refs\"}}");
    if (!resp || strstr(resp, "\"ok\":true") == NULL ||
        !response_or_stream_contains(e, resp, token, last_branch)) {
    ...
    resp = ge_run_json(e, "{\"op\":\"tips\",\"args\":{\"client_token\":\"abi-large-refs\"}}");
    if (!resp || strstr(resp, "\"ok\":true") == NULL ||
        !response_or_stream_contains(e, resp, token, last_branch)) {
    ...
    resp =
        ge_run_json(e, "{\"op\":\"push.prepare\",\"args\":{\"client_token\":\"abi-large-refs\"}}");
    if (!resp || strstr(resp, "\"ok\":true") == NULL ||
        !response_or_stream_contains(e, resp, token, last_branch)) {
    ...
    resp = ge_run_json(e, "{\"op\":\"remote\",\"args\":{\"action\":\"list\","
                          "\"client_token\":\"abi-large-refs\"}}");
    if (!resp || strstr(resp, "\"ok\":true") == NULL ||
        !response_or_stream_contains(e, resp, token, last_remote)) {
...
static int test_log_bounds(ge_engine *e) {
    ...
    for (int i = 0; i < 3; i++) {
        ...
        if (expect_ok(e, req))
            return 1;
        snprintf(req, sizeof(req), "{\"op\":\"add\",\"args\":{\"path\":\"logb%d.txt\"}}", i);
        if (expect_ok(e, req))
            return 1;
        snprintf(req, sizeof(req),
                 "{\"op\":\"commit\",\"args\":{"
...
static int test_status_porcelain(ge_engine *e) {
    if (expect_ok(e,
                  "{\"op\":\"write\",\"args\":{\"path\":\"porc_new.txt\",\"content\":\"n\\n\"}}"))
        return 1;
...
static int test_submodule_list_only(ge_engine *e) {
    ...
    if (expect_fail(e, "{\"op\":\"submodule\",\"args\":{\"action\":\"update\"}}", "host_call"))
        return 1;
    if (expect_fail(e, "{\"op\":\"submodule\",\"args\":{\"action\":\"init\"}}", "orchestrator"))
        return 1;
    if (expect_fail(e, "{\"op\":\"submodule\",\"args\":{\"action\":\"add\"}}", "host_call"))
        return 1;
    if (expect_fail(e, "{\"op\":\"submodule\",\"args\":{\"action\":\"clone\"}}", "orchestrator"))
        return 1;
...
static int test_sparse_set_patterns(ge_engine *e) {
    ...
    if (expect_ok(e, "{\"op\":\"commit\",\"args\":{"
        ...
        return 1;
    if (expect_fail(e, "{\"op\":\"sparse-set\",\"args\":{\"patterns\":[\"src\",1]}}",
                    "patterns entries must be strings"))
        return 1;
    if (expect_fail(e, "{\"op\":\"sparse-set\",\"args\":{\"path\":\"src\"}}", "need patterns"))
        return 1;
    if (expect_ok(e,
                  "{\"op\":\"sparse-set\",\"args\":{\"patterns\":[\"src\",\"docs\",\"!vendor\"]}}"))
        return 1;
    char sc[4096];
...
static int test_client_token(ge_engine *e) {
    ...
    {
        char *resp =
            ge_run_json(e, "{\"op\":\"version\",\"args\":{\"client_token\":\"tok-no-result\"}}");
        if (!resp) {
    ...
    {
        char *resp = ge_run_json(e, "{\"op\":\"rev-parse\",\"args\":{\"rev\":\"HEAD\","
    ...
    {
        char *resp = ge_run_json(e, "{\"op\":\"tips\",\"args\":{\"client_token\":\"tok-array\"}}");
        if (!resp) {
    ...
    if (expect_fail(e, "{\"op\":\"version\",\"args\":{\"client_token\":\"bad/token\"}}",
                    "invalid args.client_token"))
        return 1;

    return 0;
}

static int test_strict_run_json(ge_engine *e) {
    const char *bad[] = {
        "{\"args\":{\"op\":\"reset\"}}",
        "{\"op\":\"version\",\"op\":\"reset\"}",
        "{\"op\":\"version\",\"extra\":true}",
        "{\"op\":\"version\"} trailing",
        "{\"op\":\"ver\\qion\"}",
        "{\"op\":\"ver\\u12\"}",
        "{\"op\":\"version\",\"args\":{\"x\":1,\"x\":2}}",
    };
    for (size_t i = 0; i < sizeof(bad) / sizeof(bad[0]); i++) {
        char *resp = ge_run_json(e, bad[i]);
        int failed =
            resp && strstr(resp, "\"ok\":false") != NULL && strstr(resp, "\"code\":2") != NULL;
        if (!failed)
            fprintf(stderr, "strict JSON accepted %s -> %s\n", bad[i], resp ? resp : "(null)");
        ge_free(resp);
        if (!failed)
            return 1;
    }
    {
        const char invalid_utf8[] = "{\"op\":\"ver\xffsion\"}";
        char *resp = ge_run_json(e, invalid_utf8);
        int failed =
            resp && strstr(resp, "\"ok\":false") != NULL && strstr(resp, "\"code\":2") != NULL;
        if (!failed)
            fprintf(stderr, "strict JSON accepted invalid UTF-8 -> %s\n", resp ? resp : "(null)");
        ge_free(resp);
        if (!failed)
            return 1;
    }
    if (expect_ok(e, "{\"op\":\"\\u0076ersion\",\"args\":{}}"))
        return 1;
    return 0;
}
...
int main(void) {
    ...
    ge_engine *unsafe = ge_open("/");
    if (unsafe) {
        fprintf(stderr, "ge_open accepted the filesystem root\n");
        ge_close(unsafe);
        return 1;
    }
    char linked_root[4096];
    int linked_n = snprintf(linked_root, sizeof(linked_root), "%s-link", dir);
    if (linked_n < 0 || (size_t)linked_n >= sizeof(linked_root) || symlink(dir, linked_root) != 0)
        return 1;
    unsafe = ge_open(linked_root);
    unlink(linked_root);
    if (unsafe) {
        fprintf(stderr, "ge_open accepted a symlinked worktree root\n");
        ge_close(unsafe);
        return 1;
    }
    ...
    if (expect_ok(e, "{\"op\":\"init\"}"))
        goto fail;
    if (expect_fail(e,
                    "{\"op\":\"write\",\"args\":{\"path\":\".git/config\","
                    "\"content\":\"guest must not mutate metadata\"}}",
                    "path"))
        goto fail;
    if (expect_ok(e, "{\"op\":\"write\",\"args\":{\"path\":\"hello.txt\","
    ...
    if (expect_fail(e, "{\"op\":\"fetch.apply\",\"args\":{}}", "fetch.apply"))
        goto fail;
    if (expect_fail(e,
                    "{\"op\":\"fetch.apply\",\"args\":{\"name\":\"refs/heads/main\","
                    "\"hash\":\"0000000000000000000000000000000000000000\","
                    "\"remote\":\"bad\\nname\"}}",
                    "bad remote name"))
        goto fail;

    /* path escape remains rejected */
    if (expect_fail(e, "{\"op\":\"write\",\"args\":{\"path\":\"../x\",\"content\":\"n\"}}", NULL))
        goto fail;
    ...
    /* truncated stdout + token-scoped stream_path */
