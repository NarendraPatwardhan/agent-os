...
export interface SidecarInstance {
  ...
  kind: string;
  generation: number;
  state: "allocating" | "starting" | "ready" | "suspended" | "failed" | "closing" | "closed" | "detached";
  createdAtMs: number;
  expiresAtMs: number;
  metadata: Uint8Array;
}
...
export class VmSidecars {
  ...
  capabilities(): Promise<SidecarCapability[]> { return this.backend.capabilities(); }
  ...
  create(request: SidecarCreateRequest): Promise<SidecarInstance> { return this.backend.create(request); }
  retrieve(id: string): Promise<SidecarInstance> { return this.backend.retrieve(id); }
  list(kind?: string): Promise<SidecarInstance[]> { return this.backend.list(kind); }
  invoke(request: SidecarInvokeRequest): Promise<Uint8Array> { return this.backend.invoke(request); }
  delete(id: string): Promise<void> { return this.backend.delete(id); }
...
export function ownSidecarDescriptors(
  ...
  return descriptors
    ? Object.fromEntries(Object.entries(descriptors).map(([name, descriptor]) => [name, ownSidecarDescriptor(descriptor)]))
    : undefined;
}
...
export function validateSidecarDescriptor(
  ...
  if (contract.kind !== grant.kind || contract.version !== grant.version || contract.digest !== grant.contractDigest) {
    throw new SidecarError(SIDECAR_ERROR_CONTRACT_MISMATCH, `sidecar grant '${name}' does not match its contract descriptor`);
  }
  if (typeof grant.contractDigest !== "string" || enc.encode(grant.contractDigest).length === 0 || enc.encode(grant.contractDigest).length > SIDECAR_MAX_DIGEST_BYTES) {
    throw new SidecarError(SIDECAR_ERROR_INVALID_REQUEST, `sidecar grant '${name}' has an invalid contract digest`);
  }
  if (!Number.isInteger(grant.version) || grant.version < 1) {
    throw new SidecarError(SIDECAR_ERROR_INVALID_REQUEST, `sidecar grant '${name}' has an invalid version`);
  }
  if (!Number.isInteger(grant.maxInstances) || grant.maxInstances < 1 || grant.maxInstances > SIDECAR_MAX_INSTANCES_PER_GRANT) {
    throw new SidecarError(SIDECAR_ERROR_LIMIT, `sidecar grant '${name}' maxInstances must be 1..${SIDECAR_MAX_INSTANCES_PER_GRANT}`);
  }
  if (!(grant.config instanceof Uint8Array) || grant.config.length > SIDECAR_MAX_REQUEST_BYTES) {
    throw new SidecarError(SIDECAR_ERROR_LIMIT, `sidecar grant '${name}' configuration is too large`);
  }
  if (grant.fork !== "omit") {
    throw new SidecarError(SIDECAR_ERROR_UNSUPPORTED_FORK_POLICY, `sidecar grant '${name}' requests clone semantics that are not implemented`);
  }
  if (runtime === "embedded" && descriptor.host === undefined) {
    throw new SidecarError(SIDECAR_ERROR_HOST_MISSING, `embedded sidecar grant '${name}' requires a host alias`);
  }
  if (runtime === "remote" && descriptor.host !== undefined) {
    throw new SidecarError(SIDECAR_ERROR_INVALID_REQUEST, `remote sidecar grant '${name}' must not select a host`);
  }
  if (descriptor.host !== undefined) validateName("host alias", descriptor.host, SIDECAR_MAX_NAME_BYTES);
}
...
function validateName(label: string, value: string, max: number, dotted = false): void {
  ...
  if (bytes.length === 0 || bytes.length > max || !pattern.test(value)) {
    throw new SidecarError(SIDECAR_ERROR_INVALID_REQUEST, `${label} '${value}' is not a valid bounded identifier`);
  }
}

function validateInvoke(request: SidecarInvokeRequest): void {
  if (!boundedOpaqueId(request.id) || !Number.isSafeInteger(request.generation) || request.generation < 1) {
  ...
  if (!(request.body instanceof Uint8Array) || request.body.length > SIDECAR_MAX_REQUEST_BYTES) throw new SidecarError(SIDECAR_ERROR_LIMIT, "sidecar request is too large");
  if (request.idempotencyKey !== undefined) {
    const length = enc.encode(request.idempotencyKey).length;
    if (length === 0 || length > SIDECAR_MAX_IDEMPOTENCY_BYTES) {
      throw new SidecarError(SIDECAR_ERROR_INVALID_REQUEST, "sidecar idempotency key is outside the contract bounds");
    }
  }
...
export class EmbeddedSidecarBackend implements SidecarBackend {
  ...
  private readonly creates = new Map<string, { grant: string; kind: string; body: Uint8Array; id: string }>();
  ...
  static async attach(
    ...
    if (Object.keys(hosts).length > SIDECAR_MAX_HOSTS) throw new SidecarError(SIDECAR_ERROR_LIMIT, "too many sidecar hosts");
    if (Object.keys(descriptors).length > SIDECAR_MAX_GRANTS) throw new SidecarError(SIDECAR_ERROR_LIMIT, "too many sidecar grants");
    ...
    try {
      ...
      for (const [name, raw] of Object.entries(descriptors)) {
        ...
        if (!hosts[descriptor.host!]) throw new SidecarError(SIDECAR_ERROR_HOST_MISSING, `unknown sidecar host '${descriptor.host}'`);
  ...
  async capabilities(): Promise<SidecarCapability[]> {
    const descriptions = await Promise.all(Object.values(this.hosts).map((host) => boundedOperation(
      () => host.describe(),
      SIDECAR_DEFAULT_OPERATION_TIMEOUT_MS,
      this.abort.signal,
    )));
  ...
  async enable(name: string, descriptor: SidecarGrantDescriptor): Promise<void> {
    await this.admit(undefined, async () => {
      ...
      if (this.grants.has(name)) throw new SidecarError(SIDECAR_ERROR_GRANT_EXISTS, `sidecar grant '${name}' is already enabled`);
      const host = this.hosts[owned.host!];
      if (!host) throw new SidecarError(SIDECAR_ERROR_HOST_MISSING, `unknown sidecar host '${owned.host}'`);
      const description = await boundedOperation(
        () => host.describe(),
        SIDECAR_DEFAULT_OPERATION_TIMEOUT_MS,
        this.abort.signal,
  ...
  async disable(name: string, destroy = false): Promise<void> {
    await this.admit(undefined, async () => {
      ...
      if (!bound) throw new SidecarError(SIDECAR_ERROR_GRANT_MISSING, `sidecar grant '${name}' is not enabled`);
      const active = [...this.instances.values()].some((instance) => instance.grant === name && instance.state !== "closed");
      if (active && !destroy) throw new SidecarError(SIDECAR_ERROR_IN_USE, `sidecar grant '${name}' has active instances`);
      await boundedOperation(
      ...
      if (destroy) for (const [id, instance] of this.instances) if (instance.grant === name) {
  ...
  async create(request: SidecarCreateRequest): Promise<SidecarInstance> {
    return this.admit(undefined, async () => {
      ...
      if (!bound || bound.descriptor.grant.kind !== owned.kind) throw new SidecarError(SIDECAR_ERROR_GRANT_MISSING, `sidecar grant '${owned.grant}' is unavailable`);
      const prior = this.creates.get(owned.idempotencyKey);
      if (prior) {
        if (prior.grant !== owned.grant || prior.kind !== owned.kind || !bytesEqual(prior.body, owned.body)) {
          throw new SidecarError(SIDECAR_ERROR_IDEMPOTENCY_CONFLICT, "sidecar idempotency key was reused for different create content");
        }
      ...
      if (pending) {
        if (pending.grant !== owned.grant || pending.kind !== owned.kind || !bytesEqual(pending.body, owned.body)) {
          throw new SidecarError(SIDECAR_ERROR_IDEMPOTENCY_CONFLICT, "sidecar idempotency key was reused for different create content");
        }
  ...
  async retrieve(id: string): Promise<SidecarInstance> {
    return this.admit(id, async () => {
      if (!boundedOpaqueId(id)) throw new SidecarError(SIDECAR_ERROR_INVALID_REQUEST, "sidecar id is invalid");
      ...
      if (instance.id !== id) throw new SidecarError(SIDECAR_ERROR_PROVIDER_FAILED, "sidecar provider changed instance identity");
  ...
  async list(kind?: string): Promise<SidecarInstance[]> {
    return this.admit(undefined, async () => {
      ...
      const results = await Promise.all(authorities.map((authority) => boundedOperation(
        () => authority.list(kind),
        SIDECAR_DEFAULT_OPERATION_TIMEOUT_MS,
        this.abort.signal,
      )));
      ...
      for (let index = 0; index < results.length; index += 1) for (const instance of results[index]!) {
        const bound = this.grants.get(instance.grant);
        if (!bound || bound.authority !== authorities[index]) throw new SidecarError(SIDECAR_ERROR_PROVIDER_FAILED, "sidecar provider returned an instance outside its authority scope");
        validateInstance(instance, instance.grant, bound.descriptor.grant.kind);
        if (seen.has(instance.id)) throw new SidecarError(SIDECAR_ERROR_PROVIDER_FAILED, `sidecar providers returned duplicate instance id '${instance.id}'`);
  ...
  async invoke(request: SidecarInvokeRequest): Promise<Uint8Array> {
    return this.admit(request.id, async () => {
      ...
      if (!known) throw new SidecarError(SIDECAR_ERROR_DETACHED, `sidecar '${owned.id}' is detached`);
      if (known.grant !== owned.grant || known.kind !== owned.kind) throw new SidecarError(SIDECAR_ERROR_INVALID_REQUEST, "sidecar call does not match the bound instance");
      if (known.generation !== owned.generation) throw new SidecarError(SIDECAR_ERROR_STALE_GENERATION, `sidecar '${owned.id}' generation is stale`);
      if (known.state !== "ready") throw new SidecarError(SIDECAR_ERROR_NOT_READY, `sidecar '${owned.id}' is not ready`);
      ...
      if (result.length > SIDECAR_MAX_RESULT_BYTES) throw new SidecarError(SIDECAR_ERROR_LIMIT, "sidecar result is too large");
      return result;
    });
  }
  ...
  async beginFork(): Promise<SidecarForkBarrier> {
    if (!this.accepting) throw new SidecarError(SIDECAR_ERROR_CLOSING, "sidecar admission is closed");
  ...
  async close(): Promise<void> {
    ...
    await Promise.allSettled(authorities.map((authority) => boundedOperation(
      () => authority.close(),
      SIDECAR_DEFAULT_OPERATION_TIMEOUT_MS,
    )));
  }
  ...
  async handleGuestCall(body: Uint8Array, signal?: AbortSignal): Promise<Uint8Array> {
    try {
      if (body.length < 3) throw new SidecarError(SIDECAR_ERROR_INVALID_REQUEST, "sidecar request is truncated");
      ...
      if (message === SIDECAR_CREATE_MSG_ID) {
        ...
        return encodeSidecarResult({ ok: true, body: encodeSidecarInstance(instanceToContract(instance)) });
      }
      ...
      if (message === SIDECAR_LIST_MSG_ID) {
        ...
        this.requireGuestGrant(request.grant, request.kind);
        const instances = (await this.list(request.kind)).filter((instance) => instance.grant === request.grant);
        return encodeSidecarResult({ ok: true, body: encodeSidecarInstances({ items: instances.map(instanceToContract) }) });
      }
      ...
      throw new SidecarError(SIDECAR_ERROR_INVALID_REQUEST, `unsupported sidecar message ${message}`);
    } catch (error) {
      const sidecar = asSidecarError(error);
      return encodeSidecarResult({
        ok: false,
        body: new Uint8Array(),
        error: { code: sidecar.code, message: sidecar.message, retryable: sidecar.retryable, details: sidecar.details },
      });
    }
  }
  ...
  private requireGuestGrant(name: string, kind: string): BoundGrant {
    const bound = this.grants.get(name);
    if (!bound || bound.descriptor.grant.kind !== kind) throw new SidecarError(SIDECAR_ERROR_GRANT_MISSING, `sidecar grant '${name}' is unavailable`);
    if (!bound.descriptor.grant.guest) throw new SidecarError(SIDECAR_ERROR_PERMISSION_DENIED, `sidecar grant '${name}' is not guest-enabled`);
    return bound;
  }

  private async admit<T>(instanceId: string | undefined, operation: () => Promise<T>): Promise<T> {
    if (!this.accepting || this.abort.signal.aborted) throw new SidecarError(SIDECAR_ERROR_CLOSING, "sidecar admission is closed");
    if (this.inflight >= SIDECAR_MAX_INFLIGHT_PER_VM) throw new SidecarError(SIDECAR_ERROR_LIMIT, "sidecar in-flight limit reached");
    if (instanceId !== undefined && (this.inflightByInstance.get(instanceId) ?? 0) >= SIDECAR_MAX_INFLIGHT_PER_INSTANCE) {
      throw new SidecarError(SIDECAR_ERROR_LIMIT, `sidecar '${instanceId}' in-flight limit reached`);
    }
    this.inflight += 1;
    if (instanceId !== undefined) this.inflightByInstance.set(instanceId, (this.inflightByInstance.get(instanceId) ?? 0) + 1);
    try {
      ...
      if (instanceId !== undefined) {
        ...
        if (next === 0) this.inflightByInstance.delete(instanceId); else this.inflightByInstance.set(instanceId, next);
      }
      if (this.inflight === 0) for (const resolve of this.drained.splice(0)) resolve();
    }
  }

  private async provision(bound: BoundGrant, request: SidecarCreateRequest): Promise<SidecarInstance> {
    const live = [...this.instances.values()].filter((instance) => instance.state !== "closed");
    const pending = [...this.creating.values()].filter((item) => item.grant === request.grant).length;
    ...
    if (
      ...
      live.filter((instance) => instance.grant === request.grant).length + pending >= bound.descriptor.grant.maxInstances ||
    ...
    try {
      ...
      if (this.instances.has(instance.id)) throw new SidecarError(SIDECAR_ERROR_PROVIDER_FAILED, `sidecar provider reused instance id '${instance.id}'`);
    } catch (error) {
      await this.cleanupRejectedInstance(bound.authority, instance);
      throw error;
    }
  ...
  private async cleanupRejectedInstance(authority: SidecarAuthority, value: unknown): Promise<void> {
    if (!value || typeof value !== "object") return;
    const id = (value as { id?: unknown }).id;
    if (typeof id !== "string" || !boundedOpaqueId(id) || this.instances.has(id)) return;
    try { await authority.delete(id); } catch { /* authority close is the cleanup backstop */ }
  }

  private ensureNotClosed(): void {
    if (this.abort.signal.aborted) throw new SidecarError(SIDECAR_ERROR_CLOSING, "sidecar admission is closed");
  }
}
...
export async function forkRemoteVm(
  ...
  if (typeof body.vm?.id !== "string") throw new SidecarError(SIDECAR_ERROR_PROVIDER_FAILED, "remote VM fork response was malformed");
  if (!Array.isArray(body.warnings)) throw new SidecarError(SIDECAR_ERROR_PROVIDER_FAILED, "remote VM fork warnings were malformed");
...
export function remoteSidecars(options: RemoteSidecarHostOptions): SidecarHost {
  ...
  return {
    async describe() {
      ...
      if (!response.ok) throw await httpError(response, `sidecar capability discovery failed: ${response.status}`);
      const body = await boundedJson(response) as { sidecars?: SidecarCapability[] };
      return { kinds: capabilitiesFromWire(body.sidecars) };
    },
    async attach(context) {
      const grants = Object.entries(context.grants).map(([name, grant]) => grantToWire(name, grant));
      const response = await boundedFetch(fetcher, `${base}/v1/sidecar-scopes`, {
        method: "POST",
        headers: { ...headers, "content-type": "application/json" },
        body: JSON.stringify({ clientRef: context.vmId, grants, leaseTtlMs: SIDECAR_DEFAULT_LEASE_TTL_MS }),
        signal: context.signal,
      });
      if (!response.ok) throw await httpError(response, `sidecar scope attach failed: ${response.status}`);
      const scope = scopeFromWire(await boundedJson(response));
      return new RemoteScopeAuthority(base, headers, scope.id, scope.token, fetcher, context.signal);
    },
  };
}
...
class RemoteScopeAuthority implements SidecarAuthority {
  ...
  private url(path = ""): string { return `${this.base}/v1/sidecar-scopes/${encodeURIComponent(this.scope)}${path}`; }
  private auth(): Record<string, string> {
    return { ...this.headers, [SIDECAR_SCOPE_HEADER]: this.scopeToken };
  }
  async enable(name: string, grant: SidecarGrant): Promise<void> { await this.json(`/grants/${encodeURIComponent(name)}`, "PUT", grantToWire(name, grant)); }
  async disable(name: string, destroy = false): Promise<void> { await this.json(`/grants/${encodeURIComponent(name)}?destroy=${destroy}`, "DELETE"); }
  async create(request: SidecarCreateRequest): Promise<SidecarInstance> {
    validateCreate(request);
    return instanceFromWire(await this.json("/sidecars", "POST", createToWire(request), request.signal, request.timeoutMs));
  }
  async retrieve(id: string): Promise<SidecarInstance> { return instanceFromWire(await this.json(`/sidecars/${encodeURIComponent(id)}`, "GET")); }
  async list(kind?: string): Promise<SidecarInstance[]> {
  ...
  async invoke(request: SidecarInvokeRequest): Promise<Uint8Array> {
    validateInvoke(request);
    const body = await this.json(`/sidecars/${encodeURIComponent(request.id)}/operations/${encodeURIComponent(request.operation)}`, "POST", callToWire(request), request.signal, request.timeoutMs) as { ok?: unknown; bodyBase64?: unknown; error?: unknown };
    if (body.ok !== true) throw errorFromWire(body.error);
    return resultBodyFromWire(body.bodyBase64);
  }
  async delete(id: string): Promise<void> { await this.json(`/sidecars/${encodeURIComponent(id)}`, "DELETE"); }
  async close(): Promise<void> {
    if (this.closed) return;
    this.closed = true;
    if (this.renewTimer !== undefined) clearTimeout(this.renewTimer);
    this.renewAbort?.abort();
    try { await this.json("", "DELETE"); } catch { /* lease expiry is the cleanup backstop */ }
  }
  private async renew(): Promise<void> {
    ...
    this.renewAbort = abort;
    try {
      const scope = scopeFromWire(await this.json(
        "/renew",
        "POST",
        { leaseTtlMs: SIDECAR_DEFAULT_LEASE_TTL_MS },
        abort.signal,
      ));
      if (scope.id !== this.scope) throw new SidecarError(SIDECAR_ERROR_PROVIDER_FAILED, "sidecar scope renewal changed identity");
  ...
  private async json(path: string, method: string, body?: unknown, signal?: AbortSignal, timeoutMs?: number): Promise<any> {
    if (this.closed && method !== "DELETE") throw new SidecarError(SIDECAR_ERROR_CLOSING, "sidecar scope is closed");
    const response = await boundedFetch(this.fetcher, this.url(path), {
      method,
      headers: body === undefined ? this.auth() : { ...this.auth(), "content-type": "application/json" },
      ...(body === undefined ? {} : { body: JSON.stringify(body) }),
      ...(signal ? { signal } : {}),
    }, timeoutMs);
    if (!response.ok) throw await httpError(response, `sidecar authority request failed: ${response.status}`);
...
export class RemoteVmSidecarBackend implements SidecarBackend {
  ...
  private url(path = ""): string { return `${this.endpoint.replace(/\/$/u, "")}/v1/vms/${encodeURIComponent(this.vmId)}${path}`; }
  async capabilities(): Promise<SidecarCapability[]> {
    const response = await boundedFetch(globalThis.fetch, `${this.endpoint.replace(/\/$/u, "")}/v1/capabilities`, { headers: this.headers });
    if (!response.ok) throw await httpError(response, `sidecar capability discovery failed: ${response.status}`);
    const body = await boundedJson(response) as { sidecars?: SidecarCapability[] };
  ...
  async enable(name: string, descriptor: SidecarGrantDescriptor): Promise<void> {
    ...
    await this.json(`/sidecar-grants/${encodeURIComponent(name)}`, "PUT", grantToWire(name, owned.grant));
  }
  async disable(name: string, destroy = false): Promise<void> { await this.json(`/sidecar-grants/${encodeURIComponent(name)}?destroy=${destroy}`, "DELETE"); }
  async create(request: SidecarCreateRequest): Promise<SidecarInstance> {
    validateCreate(request);
    return instanceFromWire(await this.json("/sidecars", "POST", createToWire(request), request.signal, request.timeoutMs));
  }
  async retrieve(id: string): Promise<SidecarInstance> { return instanceFromWire(await this.json(`/sidecars/${encodeURIComponent(id)}`, "GET")); }
  async list(kind?: string): Promise<SidecarInstance[]> {
    const body = await this.json(`/sidecars${kind ? `?kind=${encodeURIComponent(kind)}` : ""}`, "GET") as { items?: unknown[] };
    return instancesFromWire(body.items);
  }
  async invoke(request: SidecarInvokeRequest): Promise<Uint8Array> {
    validateInvoke(request);
  ...
  async close(): Promise<void> {}
  private async json(path: string, method: string, body?: unknown, signal?: AbortSignal, timeoutMs?: number): Promise<any> {
    const response = await boundedFetch(globalThis.fetch, this.url(path), {
      method,
      headers: body === undefined ? this.headers : { ...this.headers, "content-type": "application/json" },
      ...(body === undefined ? {} : { body: JSON.stringify(body) }),
      ...(signal ? { signal } : {}),
    }, timeoutMs);
    if (!response.ok) throw await httpError(response, `remote sidecar request failed: ${response.status}`);
    return response.status === 204 ? {} : boundedJson(response);
  }
}

function assertCapability(capabilities: SidecarCapability[], grant: SidecarGrant, placement?: SidecarPlacement): SidecarCapability {
  const capability = capabilities.map(ownCapability).find((candidate) => candidate.kind === grant.kind && candidate.version === grant.version);
  if (!capability || capability.contractDigest !== grant.contractDigest) {
    throw new SidecarError(SIDECAR_ERROR_CONTRACT_MISMATCH, `sidecar host does not implement ${grant.kind} v${grant.version} with the requested contract digest`);
  }
  if (placement && !capability.placements.includes(placement)) {
    throw new SidecarError(SIDECAR_ERROR_UNAVAILABLE, `sidecar host cannot place ${grant.kind} ${placement}`);
  }
  if (capability.fork !== grant.fork) {
    throw new SidecarError(SIDECAR_ERROR_CONTRACT_MISMATCH, `sidecar host does not implement the requested ${grant.fork} fork policy for ${grant.kind}`);
  }
  if (grant.maxInstances > capability.maxInstancesPerVm) {
    throw new SidecarError(SIDECAR_ERROR_LIMIT, `sidecar grant exceeds the host limit for ${grant.kind}`);
  }
  return capability;
}
...
function validateCreate(request: SidecarCreateRequest): void {
  validateName("grant", request.grant, SIDECAR_MAX_NAME_BYTES);
  validateName("kind", request.kind, SIDECAR_MAX_KIND_BYTES, true);
  if (!(request.body instanceof Uint8Array) || request.body.length > SIDECAR_MAX_REQUEST_BYTES) throw new SidecarError(SIDECAR_ERROR_LIMIT, "sidecar create request is too large");
  ...
  if (idempotencyBytes === 0 || idempotencyBytes > SIDECAR_MAX_IDEMPOTENCY_BYTES) {
    throw new SidecarError(SIDECAR_ERROR_INVALID_REQUEST, "sidecar create requires a bounded idempotency key");
  }
  validateTimeout(request.timeoutMs);
}
...
function validateTimeout(timeoutMs: number | undefined): void {
  ...
  if (!Number.isSafeInteger(timeout) || timeout <= 0 || timeout > SIDECAR_MAX_OPERATION_TIMEOUT_MS) {
    throw new SidecarError(SIDECAR_ERROR_INVALID_REQUEST, "sidecar timeout is outside the contract bounds");
  }
}

function validateInstance(instance: SidecarInstance, grant: string, kind: string): void {
  const states: SidecarInstance["state"][] = ["allocating", "starting", "ready", "suspended", "failed", "closing", "closed", "detached"];
  if (
    !instance ||
    ...
    throw new SidecarError(SIDECAR_ERROR_PROVIDER_FAILED, "sidecar provider returned an invalid instance binding");
  }
}

function assertInstanceRequest(instance: SidecarInstance, request: { id: string; generation: number; grant: string; kind: string }): void {
  if (instance.id !== request.id || instance.generation !== request.generation || instance.grant !== request.grant || instance.kind !== request.kind) {
    throw new SidecarError(SIDECAR_ERROR_STALE_GENERATION, `sidecar '${request.id}' does not match the requested binding`);
  }
}
...
function ownCapability(value: SidecarCapability): SidecarCapability {
  if (!value || typeof value !== "object") throw new SidecarError(SIDECAR_ERROR_PROVIDER_FAILED, "sidecar capability was malformed");
  validateName("kind", value.kind, SIDECAR_MAX_KIND_BYTES, true);
  if (!Number.isSafeInteger(value.version) || value.version < 1 || typeof value.contractDigest !== "string" || enc.encode(value.contractDigest).length === 0 || enc.encode(value.contractDigest).length > SIDECAR_MAX_DIGEST_BYTES) {
    throw new SidecarError(SIDECAR_ERROR_PROVIDER_FAILED, `sidecar capability for '${value.kind}' has an invalid contract identity`);
  }
  if (!Array.isArray(value.placements) || value.placements.length === 0 || value.placements.some((item) => item !== "local" && item !== "remote")) {
    throw new SidecarError(SIDECAR_ERROR_PROVIDER_FAILED, `sidecar capability for '${value.kind}' has invalid placements`);
  }
  if (new Set(value.placements).size !== value.placements.length) {
    throw new SidecarError(SIDECAR_ERROR_PROVIDER_FAILED, `sidecar capability for '${value.kind}' repeats a placement`);
  }
  if (value.fork !== "omit" && value.fork !== "clone") throw new SidecarError(SIDECAR_ERROR_PROVIDER_FAILED, `sidecar capability for '${value.kind}' has an invalid fork policy`);
  if (!Number.isSafeInteger(value.maxInstancesPerVm) || value.maxInstancesPerVm < 1 || value.maxInstancesPerVm > SIDECAR_MAX_INSTANCES_PER_VM) {
    throw new SidecarError(SIDECAR_ERROR_PROVIDER_FAILED, `sidecar capability for '${value.kind}' has an invalid instance limit`);
  }
  return { ...value, placements: [...new Set(value.placements)] };
}
function ownInstance(value: SidecarInstance): SidecarInstance { return { ...value, metadata: value.metadata.slice() }; }
function randomId(prefix: string): string { return `${prefix}_${crypto.randomUUID().replace(/-/gu, "")}`; }
...
function errorFromWire(value: unknown): SidecarError {
  ...
  const details = typeof error.detailsBase64 === "string" ? base64Bytes(error.detailsBase64) : undefined;
  ...
  return new SidecarError(String(error.code ?? SIDECAR_ERROR_PROVIDER_FAILED), String(error.message ?? "sidecar operation failed"), error.retryable === true, details);
}
function warningFromWire(value: unknown): VmWarning {
  ...
  if (typeof warning.message !== "string") throw new SidecarError(SIDECAR_ERROR_PROVIDER_FAILED, "sidecar warning response was malformed");
  const hasSidecar = typeof warning.kind === "string" && typeof warning.grant === "string" && typeof warning.id === "string";
  const hasPartialSidecar = warning.kind !== undefined || warning.grant !== undefined || warning.id !== undefined;
  if (hasPartialSidecar && !hasSidecar) throw new SidecarError(SIDECAR_ERROR_PROVIDER_FAILED, "sidecar warning binding was malformed");
  return { code, message: warning.message, ...(hasSidecar ? { sidecar: { kind: warning.kind as string, grant: warning.grant as string, id: warning.id as string } } : {}) };
}
function grantToWire(name: string, grant: SidecarGrant): Record<string, unknown> { return { name, kind: grant.kind, version: grant.version, contractDigest: grant.contractDigest, guest: grant.guest, maxInstances: grant.maxInstances, fork: grant.fork === "clone" ? 2 : 1, configBase64: bytesBase64(grant.config) }; }
function createToWire(request: SidecarCreateRequest): Record<string, unknown> { return { grant: request.grant, kind: request.kind, bodyBase64: bytesBase64(request.body), idempotencyKey: request.idempotencyKey, timeoutMs: request.timeoutMs ?? SIDECAR_DEFAULT_OPERATION_TIMEOUT_MS }; }
function callToWire(request: SidecarInvokeRequest): Record<string, unknown> { return { id: request.id, generation: request.generation, grant: request.grant, kind: request.kind, operation: request.operation, bodyBase64: bytesBase64(request.body), idempotencyKey: request.idempotencyKey, timeoutMs: request.timeoutMs ?? SIDECAR_DEFAULT_OPERATION_TIMEOUT_MS }; }
function instanceFromWire(value: unknown): SidecarInstance {
  const item = value && typeof value === "object" ? value as Record<string, unknown> : {};
  if (typeof item.id !== "string" || typeof item.grant !== "string" || typeof item.kind !== "string" || typeof item.generation !== "number") throw new SidecarError(SIDECAR_ERROR_PROVIDER_FAILED, "sidecar instance response was malformed");
  if (typeof item.state !== "number" || typeof item.createdAtMs !== "number" || typeof item.expiresAtMs !== "number" || typeof item.metadataBase64 !== "string") {
    throw new SidecarError(SIDECAR_ERROR_PROVIDER_FAILED, "sidecar instance response was malformed");
  }
  ...
  if (state === undefined) throw new SidecarError(SIDECAR_ERROR_PROVIDER_FAILED, "sidecar instance response had an unknown state");
  const instance = { id: item.id, grant: item.grant, kind: item.kind, generation: item.generation, state, createdAtMs: item.createdAtMs, expiresAtMs: item.expiresAtMs, metadata: base64Bytes(item.metadataBase64) };
...
function base64Bytes(value: string): Uint8Array {
  if (!/^(?:[A-Za-z0-9+/]{4})*(?:[A-Za-z0-9+/]{2}==|[A-Za-z0-9+/]{3}=)?$/u.test(value)) {
    throw new SidecarError(SIDECAR_ERROR_PROVIDER_FAILED, "sidecar response contained invalid base64");
  }
  const buffer = (globalThis as any).Buffer;
...
async function httpError(response: Response, fallback: string): Promise<SidecarError> {
  const body = await boundedJson(response).catch(() => undefined) as { error?: unknown } | undefined;
  if (body?.error !== undefined) {
    const error = errorFromWire(body.error);
    return new SidecarError(error.code, error.message, error.retryable || response.status >= 500, error.details);
  }
...
async function boundedJson(response: Response): Promise<unknown> {
  ...
  if (!reader) {
    ...
    try { return JSON.parse(text); } catch { throw new SidecarError(SIDECAR_ERROR_PROVIDER_FAILED, "sidecar response was not valid JSON"); }
  }
...
async function boundedFetch(
  ...
  if (external?.aborted) abort.abort(); else external?.addEventListener("abort", cancel, { once: true });
  ...
  try {
    ...
    if (external?.aborted) throw new SidecarError(SIDECAR_ERROR_CANCELLED, "sidecar request was cancelled");
    throw error;
  } finally {
    clearTimeout(timer);
...
function capabilitiesFromWire(value: unknown): SidecarCapability[] {
  ...
  if (!Array.isArray(value)) throw new SidecarError(SIDECAR_ERROR_PROVIDER_FAILED, "sidecar capability response was malformed");
  return value.map((value) => {
    ...
    if (
      ...
      throw new SidecarError(SIDECAR_ERROR_PROVIDER_FAILED, "sidecar capability response was malformed");
    }
...
function instancesFromWire(value: unknown): SidecarInstance[] {
  if (!Array.isArray(value)) throw new SidecarError(SIDECAR_ERROR_PROVIDER_FAILED, "sidecar instance list response was malformed");
