...
try {
  ...
  writeFileSync(join(upstream, ".gitmodules"),
    "[submodule \"deps/lib\"]\n\tpath = deps/lib\n\turl = ../sub.git\n");
  ...
  const server = createServer(async (request, response) => {
    ...
    child.on("close", (code) => {
      ...
      if (boundary < 0) { response.statusCode = 500; response.end("invalid CGI response"); return; }
  ...
  try {
    const engine = await GitEngine.load({ identity: { name: "Wasm Test", email: "wasm@example.invalid" } });
    try {
      ...
      const initialHead = (await engine.run({ op: "rev-parse", args: { rev: "HEAD" } })).stdout?.trim();
      ...
      const updatedSubmodule = await pump.handle({ op: "submodule", args: { action: "update", path: "deps/lib" } });
      assert.equal(updatedSubmodule.ok, true, JSON.stringify(updatedSubmodule));
      assert.equal(new TextDecoder().decode(await engine.fileRead("deps/lib/SUBMODULE")), "nested wasm\n");
      ...
      assert.equal((await pump.handle({ op: "push", args: { url, refspecs: ["refs/heads/main"] } })).ok, false);
      assert.equal((await pump.handle({
        op: "push",
      ...
      const commit = await engine.run({ op: "commit", args: { message: "browser", unix_seconds: 1_700_000_001 } });
      ...
      assert.equal(git(root, ["--git-dir", remote, "rev-parse", "refs/heads/main"]), commit.stdout?.trim());
      ...
      assert.match(new TextDecoder().decode(Uint8Array.from(rawCommit.data)), new RegExp(`parent ${commit.stdout?.trim()}`));
