...
assert.equal((await engine.run({ op: "branch", args: { name: "browser-branch", delete: true } })).ok, true);
assert.equal((await engine.run({ op: "tag", args: { name: "browser-tag" } })).ok, true);
assert.equal((await engine.run({ op: "tag", args: { name: "browser-tag", delete: true } })).ok, true);
assert.equal((await engine.run({
  op: "remote",
  args: { action: "add", name: "artifact-peer", url: "https://example.invalid/repo.git" },
})).ok, true);
assert.equal((await engine.run({ op: "remote", args: { action: "get", name: "artifact-peer" } })).stdout,
  "https://example.invalid/repo.git");
assert.equal((await engine.run({ op: "remote", args: { action: "list" } })).stdout,
  "artifact-peer\thttps://example.invalid/repo.git\n");
assert.equal((await engine.run({
  op: "remote", args: { action: "remove", name: "artifact-peer" },
})).ok, true);
assert.equal((await engine.run({
  op: "config", args: { action: "set", key: "user.name", value: "Browser User" },
})).ok, true);
assert.equal((await engine.run({
  op: "config", args: { action: "set", key: "user.email", value: "browser@example.invalid" },
})).ok, true);
assert.equal((await engine.run({ op: "config", args: { action: "get", key: "user.name" } })).stdout,
  "Browser User");
assert.equal((await engine.run({ op: "config", args: { action: "list" } })).stdout,
  "user.name=Browser User\nuser.email=browser@example.invalid\n");
assert.equal((await engine.run({
  op: "config", args: { action: "set", key: "core.hooksPath", value: "/tmp/hooks" },
})).ok, false);
assert.equal((await engine.run({
  op: "config", args: { action: "remove", key: "user.email" },
})).ok, true);
assert.equal((await engine.run({ op: "config", args: { action: "list" } })).stdout,
  "user.name=Browser User\n");
...
const hostStatus = JSON.parse(await hostCall(JSON.stringify({
  op: "status",
  args: { mount: "/workspace/repo" },
})));
assert.equal(hostStatus.ok, true, "local host calls must dispatch to engine.run");
const missingMount = JSON.parse(await hostCall(JSON.stringify({
  op: "status",
  args: { mount: "/missing" },
})));
...
assert.equal(new TextDecoder().decode(await reopened.asMountDriver().open("nested/hello.txt")), "hello from sdk\n");
assert.equal((await reopened.run({ op: "rev-parse", args: { rev: "HEAD" } })).stdout, second.stdout);
