...
//!   control, zombie reaping, and the wakeups that resume WAMR-suspended guests.
...
//! Consumes: task.zig (identities/fd/caps), guest.zig (step a guest's WAMR runtime),
//!   ipc/pipe.zig (block reasons).
//! Not here: task identity/allocation (task.zig); the WAMR driver
...
pub const Scheduler = struct {
    ...
    pub fn killTask(self: *Scheduler, id: TaskId, exit_code: i32) void {
        if (self.getTask(id)) |t| {
            ...
            t.closeFd(0); // stdin
            t.closeFd(2); // stderr
            // Tearing down the WAMR instance behind `t.guest` (if any) is guest.zig's
