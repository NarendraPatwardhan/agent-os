defmodule AgentOS.Sidecars.Firecracker.Machine do
  @moduledoc false
  use GenServer, restart: :transient

  require Logger

  alias AgentOS.Sidecars.Firecracker.{ApiClient, HostHelper, Snapshot}
      ...
      case launch(id, paths, provider_opts) do
        {:ok, port} ->
          ...
          case boot(paths, provider_opts) do
            ...
            {:error, reason} ->
              close_port(port)
              HostHelper.cleanup(id, paths, %{paths: paths, provider_opts: provider_opts})
              {:stop, reason}
          end
  ...
  def handle_call({:capture, key}, _from, %{state: :ready} = state) do
    ...
    result =
      with :ok <- ApiClient.patch(state.paths.api, "/vm", %{state: "Paused"}),
    ...
    case result do
      ...
      {:error, reason} ->
        _ = ApiClient.patch(state.paths.api, "/vm", %{state: "Resumed"})
  ...
  def terminate(_reason, state) do
    ...
    if Map.get(state, :paths), do: HostHelper.cleanup(state.id, state.paths, state)
    :ok
  end
  ...
  defp prepare_paths(id, opts) do
    case Keyword.get(opts, :launch, :jailed) do
      :jailed ->
        with {:ok, paths} <- HostHelper.layout(id, opts) do
          {:ok,
           Map.merge(paths, %{
  ...
  defp launch(id, paths, opts) do
    case Keyword.get(opts, :launch, :jailed) do
      ...
      :jailed ->
        HostHelper.launch(id, paths, opts)
    end
  rescue
  ...
  defp configure(paths, opts) do
    ...
    with :ok <-
           ApiClient.put(paths.api, "/boot-source", %{
         ...
         :ok <-
           ApiClient.put(paths.api, "/machine-config", %{
             vcpu_count: vcpus,
             mem_size_mib: memory,
             smt: false
           }),
         :ok <-
           ApiClient.put(paths.api, "/vsock", %{
             guest_cid: cid,
             uds_path: paths.vsock_api
           }),
         :ok <- configure_network(paths, opts),
         :ok <- ApiClient.put(paths.api, "/actions", %{action_type: "InstanceStart"}) do
      :ok
    end
  end
  ...
  defp configure_network(paths, opts) do
    if Keyword.get(opts, :network, false) do
      ApiClient.put(paths.api, "/network-interfaces/eth0", %{
        iface_id: "eth0",
  ...
  defp restore(paths, opts) do
    ...
    ApiClient.put(paths.api, "/snapshot/load", body)
  end
  ...
  defp via(id), do: {:via, Registry, {AgentOS.SidecarRegistry, {:firecracker_machine, id}}}
end
