defmodule AgentOS.Sidecars.Firecracker.Provider do
  @moduledoc """
  ...
  defp capture_base(id, key) do
    with :ok <- Firecracker.RunnerConnection.prepare_snapshot(id),
         {:ok, _measurements} <- Firecracker.Machine.capture(id, key) do
      :ok
    end
  end
  ...
  @impl true
  def inspect(_context, %{id: id}, _opts), do: Firecracker.Machine.status(id)

  @impl true
  def renew(_context, %{id: id}, _expires_at_ms, opts), do: Firecracker.HostHelper.renew(id, opts)

  @impl true
  def invoke(_context, %{id: id}, operation, body, opts) do
    timeout = Keyword.get(opts, :timeout, 60_000)
    call_ref = Keyword.fetch!(opts, :call_ref)
    Firecracker.RunnerConnection.invoke(id, call_ref, operation, body, timeout)
  end

  @impl true
  def cancel(_context, %{id: id}, call_ref, _opts),
    do: Firecracker.RunnerConnection.cancel(id, call_ref)

  @impl true
  def delete(_context, %{id: id, supervisor: supervisor}, opts) do
    with :ok <- terminate_runner(supervisor),
         :ok <- Firecracker.HostHelper.cleanup_checked(id, opts) do
      :ok
    end
  end
  ...
  defp terminate_runner(supervisor) do
    case DynamicSupervisor.terminate_child(
           AgentOS.Sidecars.Firecracker.DynamicSupervisor,
           supervisor
         ) do
      :ok -> :ok
      {:error, :not_found} -> :ok
  ...
  def reconcile(opts) do
    ...
    with :ok <- Firecracker.HostHelper.reconcile(opts) do
      Journal.pending()
      |> Enum.filter(&MapSet.member?(kinds, Map.get(&1, :kind)))
      |> Enum.reduce_while(:ok, fn entry, :ok ->
        with :ok <- Firecracker.HostHelper.cleanup_checked(entry.id, opts),
  ...
  defp validate_launch_mode(opts) do
    case Keyword.get(opts, :launch, :jailed) do
      :jailed ->
        Firecracker.HostHelper.preflight(opts)
  ...
  defp start_runner(context, request, opts) do
    case DynamicSupervisor.start_child(
           AgentOS.Sidecars.Firecracker.DynamicSupervisor,
           {Firecracker.InstanceSupervisor,
  ...
  defp metadata(id) do
    Firecracker.RunnerConnection.metadata(id)
  catch
    :exit, reason -> {:error, {:firecracker_runner_exit, reason}}
  end

  defp log_startup(context, started_at) do
    with {:ok, daemon} <- Firecracker.Machine.measurements(context.id),
         {:ok, relay} <- Firecracker.RunnerConnection.measurements(context.id) do
      measurements =
        daemon
        |> Map.merge(relay)
