...
//!
//! kernel.wasm is a `data` dep (B1, §7.2), located via the runfiles crate as //tests/e2e does.
...
/// as //tests/e2e). The kernel lands at `_main/kernel/rust/kernel.wasm` in runfiles.
fn kernel_wasm() -> Vec<u8> {
    let r = runfiles::Runfiles::create().expect("runfiles unavailable");
    let p = r
        .rlocation("_main/kernel/rust/kernel.wasm")
