...
**syntax** is the structural-code engine, shipped in the `syntax` flavor (`syntax = loom + the parser
pack`). Parser generation stays on the host: AgentOS's Rust `mc-grammar-gen` parses an owned declarative
...
| **syntax** | + Lua/Luau structural parsers | code inspection and rewriting |
...
output, `_wasm` is the configured wasm artifact, `_release`/`_opt` is an optimized transition output, and
...
| Domain engines: SQLite (`atlas`), typst (`paper`), and syntax parsing (`syntax`) | Built |
| Images/flavors (minimal→atlas/paper/syntax), `pkgfsd`, the stamping/roster tools | Built |
