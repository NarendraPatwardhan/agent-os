...
> A thorough, ground-truth analysis of AgentOS: what each system _is_, _why_ it is shaped the
> way it is, _how_ it works, and the invariants it upholds. This document is the single design
> reference for the project. It is self-contained: it defines its own vocabulary, states the
> constitution it is judged against, and cites the code by the _names_ of things (line numbers
...
compiles to `wasm32` and runs as one `kernel.wasm`. It hosts _guest_ programs (a shell, coreutils, a
...
1. **Containment, not redaction.** A guest never receives a host object. A kernel file descriptor is
   not a host fd; a kernel pid is not a host process. The guest inhabits a _different computer_, not a
...
3. **Warm tools as libraries.** A heavy engine (SQLite, a type-checker, a document compiler) runs once
   as a _resident service_ with its state warm in linear memory; `require("sqlite")` in a script and
...
- A _mature_ Rust wasm-microkernel (the lineage called **memcontainers**) supplies the system design,
  ...
  edge and domain engines. Its one weakness was the _build_ — an imperative orchestrator with
  stale-artifact hazards, hand-staged images, and checked-in vendored libraries.
- An _early_ Zig port (the lineage called **zmc**) proves the hard part of a smaller kernel works: a
...
AgentOS keeps the design, puts _everything_ on a zero-staleness Bazel build graph with the ABI lifted
...
- **Host** — _drives_ the kernel. It loads `kernel.wasm`, implements a tiny bridge of effect
  ...
  Rust/wasmtime host and a JavaScript host — _two host families, one binary._
- **Kernel** — _is_ the OS. It compiles to `wasm32` and never runs natively, not even for tests. It
...
Derived rules also cited in code: _single source of truth per boundary_, _containment over redaction_,
_everything is a file_, _fail-closed_.
...
- **C1 — Design first; write code that teaches.** Because the system is built as much by agents as by
  people, _how_ the code is written is load-bearing. Before changing a subsystem, reason from first
  principles about the qualities that bear on _this_ change — almost always some of robustness (never
  ...
  you optimized for and justify the shape against this constitution _in the change itself_. Then write
  it literately: code whose names, structure, and comments explain the _why_ — the alternative weighed,
...
- **Identity** — _who is acting?_ A pid (`TaskId = u32`). Not authority.
- **Namespace** — _what can it see?_ A Plan-9-style per-process, copy-on-write mount table that forks on
  spawn. Not capability.
- **Capability** — _what can it do?_ An 8-bit set plus an optional confinement root, computed at exec as
...
interpreter must run in _eager_ compilation mode — lazy translation charges the guest's fuel to
...
- **No native kernel build, even for debugging.** A2. Debug by driving the real
  wasm in a host.
- **No WASI / Component-Model / bindgen on the kernel.** A4. WASI exists only as a _guest_ adapter that
  translates into `mc`.
...
This is the spine. The lineage froze its ABI as a Rust macro, which gave zero drift _within one
language_. AgentOS is polyglot (a Rust kernel and host, Zig guests, TypeScript hosts/clients, and an
Elixir control plane), so the source of truth is lifted **out of any language into data**, and the build _projects_
it into all of them.
...
symbol**, the kernel `Pending` enum variant it maps to, each argument's _kernel storage type_, the
...
(`noreturn`) and `pcall` (returns a throw code). Return _values_ are written through **out-pointer
...
- **Host-compilable output** — every table boundary projects to a Rust `macro_rules!` callback table,
  _not_ a concrete `extern` block, so the generated file carries no wasm-only attributes and validates
...
The result: one `.kdl` edit regenerates every projection, and _every_ consumer — kernel dispatch, guest
...
`unsafe impl Sync` asserted on one invariant: _no two references to a cell's interior are ever live
across a yield_. (An optional threaded build serializes every `task.step()` behind a Big Kernel Lock so
...
Capabilities are the kernel's _policy_ layer: a privileged syscall checks the bit and returns `EPERM` if
...
**Signals** are deliberately simple — there are no async handlers; _disposition_ is the whole model. A
task has a pending-signal bitset and an ignored bitset. Delivery marks the signal pending and applies it
immediately _unless the target is mid-step_ (terminating a task under its own feet is unsafe), in which
case it is applied at the next step boundary. The disposition engine handles `SIGKILL` (unconditional,
exit `128+signo`), `SIGCONT`, `SIGINT`/`SIGTERM`/`SIGHUP` (terminate by default, but an _ignored_
...
session-wide SIGHUP reaches detached grandchildren; `kill_task` closes all fd-owning objects _before_ the
...
A _step_ is driven by the pipeline driver, not the scheduler itself: pop a ready task (skipping stopped
...
lazy translation charges the _guest's_ fuel to translate a function on first call, and if a fuel slice
...
VM ceiling is set once at boot from the image manifest — "the thing Docker can't say: _runs
deterministically in ≤ N MiB._" Exhausting the lifetime fuel kills the guest with exit 137.

**The syscall suspend/resume dance** is the heart of the cooperative model. Every `mc_sys_*` host
function is _thin_: it records `Pending::Variant{ args }` into the guest's store and returns a host
...
_embedding a language with exceptions_, not of porting a C/C++ engine (§10.4 develops this).
...
**`FileSystem`** is a mountable backend — `open`, a _synchronous_ `stat` (it must never yield, because
...
`AccessDenied` (a file _mode_ bit → `EACCES`), `CrossDevice` (rename across mounts → `EXDEV`),
...
ops so identity-aware filesystems can check who acts _without the VFS layer depending on the task layer_.

### 5.2 Namespaces (`vfs/namespace.rs`)

A `Namespace` is a cheap, cloneable, Plan-9-style _view_. The mount table is an `Rc<BTreeMap<…>>` shared
with the parent on fork by a pointer copy; a `bind` or `unmount` does copy-on-write via `make_mut`, so it
affects only the acting task. Crucially, the _filesystems_ are shared `Rc<RefCell<…>>` while the _view_
...
permission on each intermediate directory _before_ lstat'ing the next component, so a symlink inside a
...
- **memfs** — the in-memory writable substrate (root fallback, `/tmp`, `/scratch`, and the overlay layer
  ...
  bumps `nlink`, and a _rename re-keys paths only_ — inodes never move, so an open fd survives a rename.
...
- **persistfs** — capability-backed persistence at `/var/persist`. The agent-visible shape of durability:
  ...
  mechanism — the host KV store — with _no host path or handle ever visible_. It is a flat whole-value KV
...
- **cowfs** — copy-on-write over a read-only base: the writable top of the root stack. A read-only base
  ...
  overlay unless tombstoned; opening a base file for write _copies it up_ (carrying its mode and
...
- **tarfs** — the read-only base image. It indexes a tar (or gzip) blob into an entry map at
  ...
  directly from the shared blob. Its header parser is the _exact byte-for-byte inverse_ of cowfs's tar
...
- **procfs** — a synthetic read-only view of kernel state at `/proc`: `uptime`, `mounts` (rendered from
  the _caller's_ namespace), and `/proc/[pid]/{cmdline,status,cwd,ctl}`. The one writable file is
...
- **envfs** — the environment as files at `/env`, per-task (POSIX): a child inherits a copy, so a
  ...
  _boot_ environment that later-exec'd agents clone.
...
only when the _last_ holder releases it. Pipes live in the scheduler's boxed pool so their heap address
...
The unifying idea, the deepest Plan-9 inheritance: a guest can _be_ a server — of a filesystem subtree,
...
return _bytes_, never a host object. This is the structural template the resident-service system refines.
...
2. **One core, never two codebases** — the CLI, the `require("…")` library, and the resident loop are all
   clients of the _same_ engine binary. The library can never do less than the command, and they cannot
...
3. **Warmth snapshots with the VM** — because the warm handle lives in linear memory, a snapshot captures
   ...
   snapshot, cannot do this. This is the single property that justifies a service _inside_ the kernel.

The lifecycle:

- **Registration** (`svc_serve`): the kernel authorizes serving a name _only for the task it activated to
  serve it_ — serve-authority is the **activation grant**, not a blanket capability — so no guest can
...
- **Connection** (`svc_connect`): mints a _session_, returns a connection fd. Routing is by
...
- **Call** (`svc_call`): a typed request blob plus optional delegated handles → a _readable result fd_
...
delegated fd numbers in a parallel buffer; `kind` is _call_ or _session-closed tombstone_. The blob is
...
application errors ride _inside_ the body). `caller_caps` is kernel-authored call-time authority metadata; a
...
next connect. The callee dies alone; it never unwinds into the caller or the kernel. Warm is _not_
...
not by `argv[0]`. Service-capability is therefore a _property, not a second artifact_: an `mc_service`
...
hosts (a browser host _cannot_ open raw sockets), so the surface is deliberately HTTP and WebSocket,
...
`/net/{http,https,ws,wss}/<host>/<path>`: _opening a path is connecting_. Scheme directories are
...
Snapshot/restore is a _host_ operation built on A8. MCSN v2 stores the kernel's linear memory—the
...
does **not** call `mc_init`: the booted state _is_ the image. It receives fresh capabilities and sinks
...
- **Browser** uses the **same** cache shape, filled **on demand**: first ready boot of a class
  ...
  when a store is passed explicitly (otherwise no surprise full capture).
- Binding is set by template fill at create, restore-from-full, restore-from-incremental (header
  base digest), or an explicit pin. Incremental capture requires a bound base present in the store
  and **never invents** a new full. Full snapshots without a store remain the default portable value.
...
the adapter is linked _into_ the tool, it runs in the tool's own linear memory — a WASI pointer is a real
...
Conformance is a _build error_, not a runtime check. **`mc-attest`** walks a finished guest's wasm import
...
a documented exclusion. The interpreter additionally gets _behavioral vectors_ (fuel exhaustion → re-park,
...
maximal-munch operators, quotes, all the substitutions, and here-docs, and deliberately does _not_
...
builtin, but a flavor _can_ layer extra `/lib/luau/*.luau` into the image that become require-able with no
...
### 10.4 Domain engines: SQLite, typst, and syntax — and the two build lanes
...
`setjmp`/`longjmp` _internally_ for OOM recovery — those jumps are in-guest and never cross the kernel
boundary. typst is built `panic = abort`: there is no unwinding to reroute; a panic **aborts the guest**,
and that is _exactly the recovery model a service wants_ — a panicking service traps → the task exits →
...
unix-dotfile VFS so all file I/O rides WASI → the adapter → `mc_sys_*` with _no custom VFS_. (A C
...
A **flavor** is a content-addressed image _and_ a product surface — a base plus domain packs, assembled by
...
tool that reads each box's applet roster and points each name at the _lowest-privilege_ box that provides
...
that emits the same sharded tree through the shared `lib/parse/bundle` emitter from a _provided_ source —
...
to load `kernel.wasm`, provide the `env` bridge, and pump `mc_tick` — and to do so _without ever making
policy_. It owns both boundaries it touches, both generated from contracts: the `env` bridge host side and
...
- **Persistence** — each key is a hex-encoded flat filename (no path traversal), `put` is atomic via
  temp+rename, and `get`/`list` return the _full_ length so the kernel can resize and retry when its probe
...
_before_ allocating a buffer sized by the untrusted length. And the two output paths are deliberately not
...
web APIs (`fetch`, `WebSocket`, `crypto`, `node:fs`/OPFS) and runs the same tick loop, so the _same_
...
because the network bridge is poll-based (a `fetch` is kicked off and drained _between_ ticks) and the run
...
curated registry's `servers` field — _our_ constant, never read from the live spec — so a user names only
...
lives in-tree (only patches and glue do); and the things that are _consumers_ of the OS (the server, the
...
is _our_ program, so it sits with the other programs under `memcontainers/programs/<tool>/`. **The server
...
leaking an implementation detail. Build stages use truthful suffixes: `_raw` is an unconfigured compiler
output, `_wasm` is the configured wasm artifact, `_release` is the compiler's release transition,
`_opt` is the Binaryen post-link output, and
...
**Naming.** **AgentOS** is the public product name. `agent-os` is the _repository and distribution
slug_; it is not a runtime prefix. The running system's identity is **`mc`**, frozen into the ABI itself —
...
path _is_ the namespace (`//memcontainers/kernel/rust`, `//memcontainers/contracts:mc_zig`).
...
1. **Containment.** A guest's entire computer — fds, pids, the VFS, the network as files — lives in linear
   ...
   _different machine_, not a filtered view of ours.
2. **Monotonic capability narrowing, enforced twice.** Authority is `parent ∩ binary ∩ requested` at exec,
   checked as an in-kernel `EPERM` at every privileged syscall — _and_ checked again at build time by
...
3. **Snapshottability as a free consequence of design.** Because all mutable OS state is in linear memory and
   ...
   _is_ the image," and warmth (a SQLite connection, a font set) rides along — gated only by an
...
- **served filesystem** — a guest that _is_ a filesystem subtree over the VFS (9P-style).
